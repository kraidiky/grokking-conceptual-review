...
- `2508.17689` · On the Edge of Memorization in Diffusion Models (Buchanan, Pai, Ma & De Bortoli, NeurIPS 2025) · внешняя работа: предмет — переход запоминание/обобщение в диффузионных моделях; гроккинг упомянут лишь строкой библиографии, где цитируется Nanda et al. 2023. Ценна тем, что даёт тому же переходу предсказанное положение (точка пересечения по параметризации) и меру, движущуюся плавно там, где наблюдаемое скачет · `data-fraction-critical-dataset-size`, `circuit-efficiency`, `progress-measures`, `phase-transition`, `double-descent` · 2026-08-28
