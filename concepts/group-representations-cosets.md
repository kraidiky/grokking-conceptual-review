...
![Косинусы частот 1–3 с раскраской точек по смежным классам (cosets): частотные признаки разрезают группу на классы (рис. 10 McCracken et al.)](assets/cosets-frequencies.png)
...
###### ref-3-5
**\[3.5\]** 2509.06931 — Shutman, Louidor & Tessler 2025, «Learning words in groups: fusion algebras, tensor ranks and Grokking». Представленческая линия продолжена двумя новыми понятиями — фузионной алгеброй группы и рангом тензора задачи: фузионные правила делают bsc-носитель тензора слова разрежённым, и именно это названо причиной способности сети выучить слово при малой ширине. [`"A key point is that the fusion structure of $G$ restricts the above set considerably"`](../papers/2509.06931.learning-words-in-groups-fusion-algebras-tensor-ranks-and-grokking/2509.06931.learning-words-in-groups-fusion-algebras-tensor-ranks-and-grokking.card.md#p10-3). *«[Ключевой момент в том, что фузионная структура $G$ значительно сужает указанное множество](../papers/2509.06931.learning-words-in-groups-fusion-algebras-tensor-ranks-and-grokking/2509.06931.learning-words-in-groups-fusion-algebras-tensor-ranks-and-grokking.card.md#p10-3)»*.

###### ref-3-6
**\[3.6\]** 2606.17399 — Nguyen 2026, «The Discrete-Log Clock: How a Transformer Learns Modular Multiplication». Базис мультипликативных характеров — фурье-преобразование на абелевой группе $(\mathbb{Z}/p\mathbb{Z})^{*}$, раскладывающее вложение по неприводимым представлениям циклической группы порядка 112: главные компоненты $W_{E}$, шумные в целочисленном порядке, в лог-порядке синусоидальны, эффективный ранг 10.3 из 112 численно согласуется с четырьмя частотами (4 частоты $\times$ sin/cos плюс постоянная — порядка 9–10; в статье эта согласованность не отмечена). Нюанс: работы о неприводимых представлениях (Chughtai, Stander) отделены как «про другие группы», хотя мультипликативные характеры — тот же аппарат для абелевой группы; довод об интерференции опирается на «4 несоизмеримые частоты», что неверно — при сдвиге $\Delta=56$ различение держится целиком на одной частоте 47. [`"In log-order, several components reveal sinusoidal structure (Figure 5), consistent with the embedding encoding multiplicative characters."`](../papers/2606.17399.the-discrete-log-clock-how-a-transformer-learns-modular-multiplication/original/2606.17399.the-discrete-log-clock-how-a-transformer-learns-modular-multiplication.md#p3-8). *«[В лог-порядке несколько компонент выявляют синусоидальную структуру (Рисунок 5), согласующуюся с кодированием вложением мультипликативных характеров.](../papers/2606.17399.the-discrete-log-clock-how-a-transformer-learns-modular-multiplication/2606.17399.the-discrete-log-clock-how-a-transformer-learns-modular-multiplication.card.md#p3-8)»*
