...
###### ref-3-6
**\[3.6\]** 2309.03800 — Edelman, Goel, Kakade, Malach & Zhang 2023, «Pareto Frontiers in Neural Feature Learning: Data, Compute, Width, and Luck». Разворачивает офлайновую $(n,k)$-чётность в многоресурсный фронт: SQ-граница связывает данные, ширину, время и вероятность успеха, и гроккинг живёт на кромке этого фронта. [`"The $(n,k)$-parity learning problem is to identify $S$ from samples $(x\sim\mathrm{Unif}(\{\pm 1\}^{n},y=\chi_{S}(x)))$"`](../papers/2309.03800.pareto-frontiers-in-neural-feature-learning-data-compute-width-and-luck/2309.03800.pareto-frontiers-in-neural-feature-learning-data-compute-width-and-luck.card.md#p4-1). *«[Задача обучения $(n,k)$-чётности — опознать $S$ по примерам $(x\sim\mathrm{Unif}(\{\pm 1\}^{n},y=\chi_{S}(x)))$](../papers/2309.03800.pareto-frontiers-in-neural-feature-learning-data-compute-width-and-luck/2309.03800.pareto-frontiers-in-neural-feature-learning-data-compute-width-and-luck.card.md#p4-1)»*\
Доп. (строгий смысл трудности): [`"it is a *maximally hard* toy problem in a rigorous sense"`](../papers/2309.03800.pareto-frontiers-in-neural-feature-learning-data-compute-width-and-luck/2309.03800.pareto-frontiers-in-neural-feature-learning-data-compute-width-and-luck.card.md#p10-1) — *«[это *максимально трудная* игрушечная задача в строгом смысле](../papers/2309.03800.pareto-frontiers-in-neural-feature-learning-data-compute-width-and-luck/2309.03800.pareto-frontiers-in-neural-feature-learning-data-compute-width-and-luck.card.md#p10-1)»*.


###### ref-3-7
**\[3.7\]** 2601.22450 — Huang, Mirzasoleiman 2026, «Tuning the Implicit Regularizer of Masked Diffusion Language Models: Enhancing Generalization via Insights from $k$-Parity». Полигон $(20,6)$-parity в новой роли — сверка целей обучения: метка приклеена последним символом последовательности, и та же задача решается либо надзором по метке (nanoGPT грокает), либо восстановлением стохастически маскированных символов (гроккинга нет, кривые сходятся вместе); доля сигнала $P_{S}=(k+1)\mathbb{E}[t(1-t)^{k}]$ управляется распределением маскирования, и быстрее всего сходятся диапазоны около теоретического оптимума $\mathcal{U}[0,0.246]$. Нюанс: против линии Barak et al. мер скрытого прогресса здесь нет вовсе — по двум кривым точности не отличить, полка убрана или лишь укорочена; собственный сигнал-оптимум следствия 4.8 ($t=1/7$ точечно) не проверен ни разу. [`"We conduct experiments on the $(n,k)=(20,6)$ parity task using a standard Transformer architecture and cross-entropy loss to validate our theoretical framework."`](../papers/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity/original/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity.md#p6-5). *«[Мы проводим опыты на задаче $(n,k)=(20,6)$-parity со стандартной трансформерной архитектурой и перекрёстно-энтропийной потерей, чтобы проверить нашу теоретическую рамку.](../papers/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity.card.md#p6-5)»*
## Цитирования
