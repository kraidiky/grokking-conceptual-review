...
###### ref-3-4
**\[3.4\]** 2605.12394 — Prakash & Martin 2026, «Detecting overfitting in Neural Networks during long-horizon grokking using Random Matrix Theory». Развёрнутая версия приборной линии: термин анти-гроккинг подан как собственное введение (прямые предшественники с теми же опытами не процитированы), а его структурное отличие от предгроккинга — ловушки корреляции в перемешанных спектрах весов — показано уже на трёх постановках, включая GPT2 на композиции знаний; появление ловушек совпадает с минимумом тестовой потери. [`"Correlation Traps reveal a structural difference between two superficially similar regimes. In both pre-grokking and anti-grokking, training accuracy can be high while test accuracy is poor. But the weight spectra are different"`](../papers/2605.12394.detecting-overfitting-in-neural-networks-during-long-horizon-grokking-using-random-matrix-theory/original/2605.12394.detecting-overfitting-in-neural-networks-during-long-horizon-grokking-using-random-matrix-theory.md#p8-5). *«[Ловушки корреляции открывают структурное различие между двумя внешне похожими режимами. И в предгроккинге, и в анти-гроккинге обучающая точность может быть высокой при плохой тестовой. Но весовые спектры различны](../papers/2605.12394.detecting-overfitting-in-neural-networks-during-long-horizon-grokking-using-random-matrix-theory/2605.12394.detecting-overfitting-in-neural-networks-during-long-horizon-grokking-using-random-matrix-theory.card.md#p8-5)»*.


###### ref-3-5
**\[3.5\]** 2509.21016 — Sun et al. 2025, «RL Grokking Recipe: How Does RL Unlock and Transfer New Algorithms in LLMs?». Независимое от линии Prakash-Martin наблюдение потери качества при продолжении обучения после сходимости — в RL-постановке: на Manufactoria-REGEX продолжение GRPO-обучения после выхода кривой на полку обрушивает качество («RL collapse»), откуда вывод о необходимости стабилизации или ранней остановки. Нюанс: одна кривая одного прогона, без разбора (ни норм весов, ни спектральных примет); параллель с супервизным анти-гроккингом в статье не проведена. [`"Continued training eventually triggers an *RL collapse*, highlighting the need for stabilization or early stopping once solutions consolidate."`](../papers/2509.21016.rl-grokking-recipe-how-does-rl-unlock-and-transfer-new-algorithms-in-llms/2509.21016.rl-grokking-recipe-how-does-rl-unlock-and-transfer-new-algorithms-in-llms.card.md#p31-2). *«[Продолжение обучения в конце концов вызывает *RL-коллапс*, подчёркивая нужду в стабилизации или ранней остановке после консолидации решений.](../papers/2509.21016.rl-grokking-recipe-how-does-rl-unlock-and-transfer-new-algorithms-in-llms/2509.21016.rl-grokking-recipe-how-does-rl-unlock-and-transfer-new-algorithms-in-llms.card.md#p31-2)»*
###### ref-3-6
**\[3.6\]** 2607.08350 — Pranjić, Roth, Tutschku 2026, «Grokking and epoch-wise double descent in quantum neural networks». Поздний спад обобщения в квантовой постановке: без якоря заметная доля прогонов после перехода дрейфует назад к высокой тестовой ошибке при застывшей обучающей потере, а слабый явный $L_2$-штраф ($\lambda=10^{-4}$) при $n_{l}>4$ сводит конечную ошибку почти к нулю, не меняя распределения $\tau_{\mathrm{g}}/\tau_{\mathrm{f}}$ — якорь удерживает найденное решение, а не ускоряет поиск. Нюанс: центральное объяснение аннотации («спад коррелирует с неограниченным ростом нормы весов») прямых свидетельств не имеет — ни норма, ни обещанная липшицева граница нигде не измерены, косвенно — лишь то, что штраф на норму спад устраняет; запутанность, названная поводом к работе, не измерена ни разу; связь с ungrokking/конкуренцией контуров не проведена, Omnigrok не цитируется. [`"In contrast, the unregularized trajectories display significant variance, with a prominent cluster of runs drifting back to high test error regimes by the end of training."`](../papers/2607.08350.grokking-and-epoch-wise-double-descent-in-quantum-neural-networks/2607.08350.grokking-and-epoch-wise-double-descent-in-quantum-neural-networks.card.md#p16-6). *«[В контрасте, нерегуляризованные траектории показывают значимую дисперсию, с выдающимся кластером прогонов, дрейфующих назад к режимам высокой тестовой ошибки к концу обучения.](../papers/2607.08350.grokking-and-epoch-wise-double-descent-in-quantum-neural-networks/2607.08350.grokking-and-epoch-wise-double-descent-in-quantum-neural-networks.card.md#p16-6)»*

###### ref-3-7
**\[3.7\]** 2608.07436 — Janati, El Maghraoui, Kanavalau, Belfatmi 2026, «Post-Grokking Collapse at the Representation–Readout Interface in Muon-Trained Transformers». Разграничение позднего сбоя по признаку, что происходит с обучающей точностью: при антигроккинге она держится совершенной, тогда как тестовая обваливается, а здесь обе падают в одном промежутке между замерами ($100\%\to 21.12\%$ и $100\%\to 19.04\%$), и обучающая потеря растёт с $1.53\times 10^{-7}$ до $57.5$ — механизм, действующий на одно обобщение, оставил бы обучающую выборку решённой. Нюанс: совпадает при этом более важное — обе линии утверждают, что поздний обвал ускользает от существующих мер прогресса, но расходятся в лечении: там предлагают спектральную статистику по весам, здесь показывают, что спектр представления как раз слеп; ближайшая по предмету 2602.02859 не цитируется вовсе. [`"A mechanism acting on generalization alone would leave the training set solved. This one does not, which points to the interface between the representation and the readout rather than to either alone."`](../papers/2608.07436.post-grokking-collapse-at-the-representation-readout-interface-in-muon-trained-transformers/original/2608.07436.post-grokking-collapse-at-the-representation-readout-interface-in-muon-trained-transformers.md#p3-1). *«[Механизм, действующий на одно обобщение, оставил бы обучающую выборку решённой. Этот не оставляет, что указывает на границу между представлением и считыванием, а не на что-либо из двух по отдельности.](../papers/2608.07436.post-grokking-collapse-at-the-representation-readout-interface-in-muon-trained-transformers/2608.07436.post-grokking-collapse-at-the-representation-readout-interface-in-muon-trained-transformers.card.md#p3-1)»*
