...
###### ref-3-3
**\[3.3\]** 2512.00686 — Lakkapragada 2025, «Using physics-inspired Singular Learning Theory to understand grokking & other phase transitions in modern neural networks». Мотивировка через сингулярность: PAC-границы объявлены пустыми, потому что нейросети неидентифицируемы, и мерой сложности взят местный коэффициент обучения (LLC); измерения дают и совпадение с теорией (квадратика низкоранговых сетей), и полку на порядок ниже ожидания у формально регулярных полиномиальных регрессоров. [`"well-studied PAC-bounds from statistical learning theory are vacuous and fail to explain the remarkable generalization power of neural networks"`](../papers/2512.00686.using-physics-inspired-singular-learning-theory-to-understand-grokking-and-other-phase-transitions-in-modern-neural-networks/original/2512.00686.using-physics-inspired-singular-learning-theory-to-understand-grokking-and-other-phase-transitions-in-modern-neural-networks.md#p1-3). *«[хорошо изученные PAC-границы из статистической теории обучения пусты и не могут объяснить замечательную обобщающую силу нейронных сетей](../papers/2512.00686.using-physics-inspired-singular-learning-theory-to-understand-grokking-and-other-phase-transitions-in-modern-neural-networks/2512.00686.using-physics-inspired-singular-learning-theory-to-understand-grokking-and-other-phase-transitions-in-modern-neural-networks.card.md#p1-3)»*.

###### ref-3-4
**\[3.4\]** 2606.05863 — Tan, Gai & Zhang 2026, «Deciphering Two Training Clocks in Grokking via Deep Linear Network Theory with Conditional ReLU Reduction». Мост от спектра к обобщению через робастность Xu—Mannor: устойчивый ранг задаёт выходно-действенную размерность, уменьшающую показатель покрывающего члена; при собственных величинах статьи граница численно пуста (показатель выходит на сотни при полутора десятках тысяч примеров), и «отложенная генерализация» выводится из улучшения границы, а не измеренной тестовой потери. [`"*Thus the action of $A^{\prime}$ on embeddings is approximated, up to relative output error $\eta$, by a $d_{\eta}$-dimensional principal subspace controlled by stable rank.*"`](../papers/2606.05863.deciphering-two-training-clocks-in-grokking-via-deep-linear-network-theory-with-conditional-relu-reduction/2606.05863.deciphering-two-training-clocks-in-grokking-via-deep-linear-network-theory-with-conditional-relu-reduction.card.md#p15-5). *«[*Тем самым действие $A^{\prime}$ на вложениях приближается, с точностью до относительной выходной ошибки $\eta$, главным $d_{\eta}$-мерным подпространством, управляемым устойчивым рангом.*](../papers/2606.05863.deciphering-two-training-clocks-in-grokking-via-deep-linear-network-theory-with-conditional-relu-reduction/2606.05863.deciphering-two-training-clocks-in-grokking-via-deep-linear-network-theory-with-conditional-relu-reduction.card.md#p15-5)»*.

###### ref-3-5
**\[3.5\]** 2607.13749 — Kam, Cadet, Bessafi, Cadet 2026, «Algebraic Representability as the Limiting Regime of Grokking: An Exactly Solvable Model with Holomorphic Activations». Редкая для корпуса нижняя граница на ОБУЧАЮЩИЕ потери: $\min L\geq\delta^{2}-O(N^{-1/2})$ с вероятностью $1-O(1/N)$, где $\delta^{2}$ — внеполосная спектральная энергия цели, и граница не зависит от ширины — запрет на само запоминание, а не только на обобщение; для $ab$ доля энергии в полосе $(k+1)/p^{2}<0.1\%$, потеря неотличима от необученной сети; для обучаемых кодирований граница переносится с заменой внеполосной энергии на хвост сингулярного спектра. Нюанс: доказательство отсылает к разбору «в приложении», но приложения в статье нет (после раздела 6 сразу литература) — равномерная граница по $g\in\mathcal{F}_{k}$ и инъективность ограничения остаются наброском; обозначение $\delta^{2}$ перегружено — та же буква означает внеполосную энергию в (6) и ранговый остаток $1-(k+1)/p$ в следствии 1. [`"a network with $H=10{,}000$ hidden units performs identically to one with $H=10$."`](../papers/2607.13749.algebraic-representability-as-the-limiting-regime-of-grokking-an-exactly-solvable-model-with-holomorphic-activations/original/2607.13749.algebraic-representability-as-the-limiting-regime-of-grokking-an-exactly-solvable-model-with-holomorphic-activations.md#p6-11). *«[сеть с $H=10{,}000$ скрытых единиц ведёт себя тождественно сети с $H=10$.](../papers/2607.13749.algebraic-representability-as-the-limiting-regime-of-grokking-an-exactly-solvable-model-with-holomorphic-activations/2607.13749.algebraic-representability-as-the-limiting-regime-of-grokking-an-exactly-solvable-model-with-holomorphic-activations.card.md#p6-11)»*
