...
через низкочастотный фильтр, *«Усиления низких частот градиентов $g(t)$ можно добиться, добавив к сигналу $g(t)$ его же, пропущенный через фильтр нижних частот»*
...
низкочастотная компонента *«градиенты, пропущенные через ФНЧ; они добавляются к текущему обновлению параметра на каждом шаге оптимизатора»*
...
###### ref-4-1
**\[4.1\]** 2509.10562 — Lopatin, Kozyrev & Pechen 2025, «Predator–Prey Model: Driven Hunt for Accelerated Grokking». Третий член линии ускорителей гроккинга: как и Grokfast и NeuralGrok, поправка вставляется между вычислением градиента и шагом, но источник иной — внешний снос от второй копии вектора параметров, вычисляемый вовсе без градиента, так что итерация не дороже базовой. Ни Grokfast, ни NeuralGrok не процитированы; общая с Grokfast непроверенная альтернатива — абляция против простого усиления момента — отсутствует и здесь. [`"one epoch of training in the graphs in Fig. 2 contains the same number of gradient calls as one epoch in the graphs in Fig. 1"`](../papers/2509.10562.predator-prey-model-driven-hunt-for-accelerated-grokking/2509.10562.predator-prey-model-driven-hunt-for-accelerated-grokking.card.md#p7-1). *«[одна эпоха обучения на графиках рис. 2 содержит то же число вызовов градиента, что и одна эпоха на графиках рис. 1](../papers/2509.10562.predator-prey-model-driven-hunt-for-accelerated-grokking/2509.10562.predator-prey-model-driven-hunt-for-accelerated-grokking.card.md#p7-1)»*.
