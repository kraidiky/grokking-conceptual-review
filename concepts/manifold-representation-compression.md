...
###### ref-3-14
**\[3.14\]** 2607.06639 — Truong Xuan Khanh 2026, «At-Grok Is Not Converged: A Measurement-Validity Audit for Grokking Representation Metrics». Величиной пост-гроккинговой задержки сжатия правит нормализация: в упряжи трёх рук с переменой по одной доля сжатия, завершённая к гроккингу (frac-pre), равна 0.87 у каноничного трансформера без LayerNorm, 0.66 у MLP и 0.25 при добавлении одного LayerNorm — задержка присутствует на каждой руке, но LayerNorm делает её большой; предрегистрированная сверка отвергает объяснение через потокенную инвариантность к масштабу (rms-рука: PD2 $-0.56$ выполнено, PD1 frac-pre$=0.90$ провалено). Нюанс: внутреннее несогласие ровно на несущей руке — рис. 5A даёт frac-pre LayerNorm-руки 1.00 против 0.25 в тексте (и Spearman $+0.13$ против $-0.80$), из-за чего порог предрегистрации не отделил бы и сам LayerNorm; упряжь малосеменна́, и самое яркое число (3.2×) автор сам называет наименее обеспеченным; побочный «закон глубины» самофальсифицирован проверками архитектуры ($+0.14$/$+0.82$ против $-1.0$) и протокола (знак разворачивается, $+1.0$). [`"within this harness it is LayerNorm, not “being a transformer,” that defers compression past grokking."`](../papers/2607.06639.at-grok-is-not-converged-a-measurement-validity-audit-for-grokking-representation-metrics/2607.06639.at-grok-is-not-converged-a-measurement-validity-audit-for-grokking-representation-metrics.card.md#p13-2). *«[внутри этой упряжи именно LayerNorm, а не «бытие трансформером», откладывает сжатие за гроккинг.](../papers/2607.06639.at-grok-is-not-converged-a-measurement-validity-audit-for-grokking-representation-metrics/2607.06639.at-grok-is-not-converged-a-measurement-validity-audit-for-grokking-representation-metrics.card.md#p13-2)»*

###### ref-3-15
**\[3.15\]** 2607.11666 — Kazanskii 2026, «How to Tame Grokking: Representation Geometry as a Control Signal». Обвал действенной размерности (отношение участия) как предвестник и рычаг: у базы обвал глубоких слоёв на ~15–25K шагов, гроккинг на ~250–500K — разрыв 264.3K шагов; несглаженные кривые показывают несколько резких одновременных перестроек $D_{\mathrm{eff}}$ и локальной соседской дистанции; GeomDR сжимает задержку «обвал — гроккинг» до 2.2–3.0K шагов, притягивая систему к диагонали $t_{\mathrm{collapse}}=t_{\mathrm{grok}}$. Нюанс: «обвал предшествует гроккингу во всех обстановках» измерено количественно только для сложения и MLP, порог «половина начальной размерности» произволен; причинность не показана — GeomDR меняет саму цель обучения, и ускорение может идти от добавленного регуляризующего давления, а не от размерности (контроля со штрафом той же силы на несвязанную величину нет); близкие работы корпуса о размерности и геометрии (2211.13239, 2605.06352, 2606.08985, 2606.32000) не задеты вовсе. [`"Figure 5 shows that dimensionality collapse consistently precedes grokking in both baseline and GeomDR models."`](../papers/2607.11666.how-to-tame-grokking-representation-geometry-as-a-control-signal/original/2607.11666.how-to-tame-grokking-representation-geometry-as-a-control-signal.md#p5-4). *«[Рисунок 5 показывает, что обвал размерности последовательно предшествует гроккингу и в базовых, и в GeomDR-моделях.](../papers/2607.11666.how-to-tame-grokking-representation-geometry-as-a-control-signal/2607.11666.how-to-tame-grokking-representation-geometry-as-a-control-signal.card.md#p5-4)»*
