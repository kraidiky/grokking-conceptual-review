...
###### ref-3-10
**\[3.10\]** 2411.03541 — Kumar, Bordelon, Pehlevan, Murthy & Gershman 2024, «Do Mice Grok? Glimpses of Hidden Progress During Overtraining in Sensory Cortex». Предсказательная сила позднего обучения признаков: выученные признаки переиспользуемы при обращении меток, откуда нейронное объяснение эффекта обращения после переобучения. Нюанс: модель — двухслойная линейная сеть с квадратичной потерей, отвязанная от маржинального довода остальной работы, а сам объясняемый эффект оспорен в литературе по видам и задачам. [`"If a network learns features to compute $y(x)$, it is possible many such features can be reused to compute $-y(x)$"`](../papers/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex/original/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex.md#p10-2). *«[Если сеть выучивает признаки для вычисления $y(x)$, возможно, многие из них можно переиспользовать для вычисления $-y(x)$](../papers/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex.card.md#p10-2)»*\
Доп. (ключ вывода): [`"Since $K_{y}(T)$ is monotonically increasing with $T$, longer training on the first task will accelerate learning of the reversed task, providing an explanation for overtraining reversal"`](../papers/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex/original/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex.md#p11-4) — *«[Поскольку $K_{y}(T)$ монотонно растёт с $T$, более долгое обучение на первой задаче ускорит выучивание обращённой задачи, что и даёт объяснение обращения после переобучения](../papers/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex.card.md#p11-4)»*.


###### ref-3-11
**\[3.11\]** 2601.22450 — Huang, Mirzasoleiman 2026, «Tuning the Implicit Regularizer of Masked Diffusion Language Models: Enhancing Generalization via Insights from $k$-Parity». Прямое продолжение энергетической рамки Tian (2509.21519): при предположении ленивого считывания минимизация MD-потери равносильна максимизации энергии $E(\boldsymbol{W})={\boldsymbol{c}}^{\top}\boldsymbol{\Sigma}^{\dagger}{\boldsymbol{c}}$, и доля сигнала $P_{S}$ входит в неё квадратично — темп обучения признаков управляется распределением маскирования; следствие 4.6 — в пределе чистого сигнала энергия достаточно выразительной сети постоянна и обучение признаков схлопывается, то есть шумовое слагаемое и держит обучение признаков живым (аналог вывода Tian о коллапсе без регуляризации). Нюанс: следствие говорит о недостижимой точке $P_{N}=0$ и молчит о малых ненулевых $P_{N}$, где идёт настоящее обучение; ленивость считывания взята как техническое удобство, тогда как в корпусе переход lazy-to-rich — сама разбираемая величина. [`"Since $E(\boldsymbol{W})\propto P_{S}^{2}$, the signal ratio $P_{S}$ acts as a dynamic gain factor."`](../papers/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity/original/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity.md#p5-9). *«[Поскольку $E(\boldsymbol{W})\propto P_{S}^{2}$, доля сигнала $P_{S}$ действует как переменный усилитель.](../papers/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity.card.md#p5-9)»*\
Доп. (коллапс): [`"Corollary 4.6 formalizes a critical failure mode where the gradients for the internal parameters $\boldsymbol{W}$ vanish when the training falls entirely into the Pure Signal Regime ($P_{N}\to 0$)."`](../papers/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity/original/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity.md#p5-12). *«[Следствие 4.6 формализует критический режим отказа, где градиенты внутренних параметров $\boldsymbol{W}$ исчезают, когда обучение целиком попадает в чистый сигнальный режим ($P_{N}\to 0$).](../papers/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity/2601.22450.tuning-the-implicit-regularizer-of-masked-diffusion-language-models-enhancing-generalization-via-insights-from-k-parity.card.md#p5-12)»*
## Цитирования
