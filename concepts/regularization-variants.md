...
###### ref-3-9
**\[3.9\]** 2605.08464 — Walker, Roddenberry, Humayun, Balestriero, Baraniuk 2026, «The Geometric Structure of Models Learning Sparse Data». GrokAlign — штраф $\mathbb{E}_{\boldsymbol{\epsilon}}(\|f(\boldsymbol{x}+\boldsymbol{\epsilon})-f(\boldsymbol{x})\|_{2}^{2}/\sigma^{2})\to\|\boldsymbol{J}_{\boldsymbol{x}}\|_{F}^{2}$, убирающий не видимые на выходе ортогональные входу составляющие якобиана; приложение E сводит к той же норме Фробениуса прямую оптимизацию согласованности и ядерную норму (все — оценщиком Хатчинсона), а выбранная реализация с одной проекцией в выходном пространстве использует уже накопленные градиенты и потому почти бесплатна. Нюанс: связь нормового ограничения теоремы 1 с weight decay объявлена аналогией, а не выведена; силы штрафа подобраны под каждую задачу отдельно; в ячейке «OrthoGrad, XOR» среднее напечатано по 20% выживших прогонов — отбор по исходу. [`"GrokAlign regularization can be interpreted as a method for removing the orthogonal components of a classifier’s Jacobians."`](../papers/2605.08464.the-geometric-structure-of-models-learning-sparse-data/original/2605.08464.the-geometric-structure-of-models-learning-sparse-data.md#p7-5). *«[регуляризацию GrokAlign можно истолковать как метод удаления ортогональных составляющих якобианов классификатора.](../papers/2605.08464.the-geometric-structure-of-models-learning-sparse-data/2605.08464.the-geometric-structure-of-models-learning-sparse-data.card.md#p7-5)»*

###### ref-3-10
**\[3.10\]** 2607.11666 — Kazanskii 2026, «How to Tame Grokking: Representation Geometry as a Control Signal». Новый класс регуляризатора — штраф на спектр ковариации активаций (не весов): $L_{\mathrm{GeomDR}}=\sum_{j>d^{*}}\mu_{j}$, с тремя доказанными свойствами цели (контроль действенного ранга, граница низкорангового приближения через Эккарта–Янга, сжатие объёма ковариационного эллипсоида — прямо оговорено, что это свойства определения, а не доказательство ускорения); в сопоставлении лучший weight decay даёт 27.2K шагов, GrokFast — 372.1K, GeomDR — 7.0K, а сочетание GrokFast+GeomDR не гроккает ни разу (0/5). Нюанс: «GrokFast улучшает относительно базы» при 372.1K против базовых 362.1K — улучшена лишь доля успеха; таблица сравнения сводит строки с разным числом прогонов (GeomDR 10/10, остальные по 5); срыв сочетания двух ускорителей — второй такой случай в корпусе (после 2607.04333, где Grokfast поверх SupCon срывал все семена) и заслуживает отдельной проверки. [`"In contrast to the individual methods, the combined configuration failed to satisfy the grokking criterion in any run within the available training budget"`](../papers/2607.11666.how-to-tame-grokking-representation-geometry-as-a-control-signal/original/2607.11666.how-to-tame-grokking-representation-geometry-as-a-control-signal.md#p21-3). *«[В контрасте с отдельными методами, комбинированная конфигурация не удовлетворила критерию гроккинга ни в одном прогоне в доступном обучающем бюджете](../papers/2607.11666.how-to-tame-grokking-representation-geometry-as-a-control-signal/2607.11666.how-to-tame-grokking-representation-geometry-as-a-control-signal.card.md#p21-3)»*
