...
[Композиция групп / некоммутативность](group-composition-non-commutative-s5.md) ← предыдущая карточка, следующая → [Гроккинг вне нейросетей](grokking-in-non-neural-models.md)
...
![Неявное рассуждение над параметрическим знанием приобретается только через гроккинг — далеко за пределами переобучения (рис. 1 Wang et al.)](assets/implicit-reasoning-grokking.png)
