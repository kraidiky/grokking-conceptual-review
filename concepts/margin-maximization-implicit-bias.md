...
Частный случай той же линии — **неявное смещение Adam** (implicit bias of Adam): у адаптивных оптимизаторов предпочтение направления иное, чем у градиентного спуска, и потому предсказания о запасе к ним переносятся не дословно.
...
###### ref-3-5
**\[3.5\]** 2411.03541 — Kumar, Bordelon, Pehlevan, Murthy & Gershman 2024, «Do Mice Grok? Glimpses of Hidden Progress During Overtraining in Sensory Cortex». Переносит маржинальную линию корпуса на живую кору: из трёх понятий максимизации маржи принято то, где признаки меняются, а декодер на каждом шаге доучивается до сходимости, и маржа SVM на пириформных данных растёт по дням переобучения. Нюанс: причинный довод — одна hinge-абляция в синтетической модели. [`"if the margin of the max-margin (converged) classifier is increasing in (3), it implies the features are evolving to allow for a max-margin classifier with larger margin"`](../papers/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex/original/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex.md#p6-2). *«[если маржа максимально-маржинального (сошедшегося) классификатора растёт в (3), это означает, что признаки эволюционируют так, чтобы допускать максимально-маржинальный классификатор с большей маржой](../papers/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex.card.md#p6-2)»*\
Доп. (итог на нейроданных): [`"PPC representations evolve to increase the margin of the maximum margin classifier trained on those representations"`](../papers/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex/original/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex.md#p7-1) — *«[представления PPC эволюционируют так, чтобы увеличивать маржу максимально-маржинального классификатора, обученного на этих представлениях](../papers/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex/2411.03541.do-mice-grok-glimpses-of-hidden-progress-during-overtraining-in-sensory-cortex.card.md#p7-1)»*.

###### ref-3-6
**\[3.6\]** 2602.12039 — Beck, Bar-Sinai & Levi 2026, «The Implicit Bias of Logit Regularization». Рамка неявного смещения расширена на класс выпуклых логит-штрафов: конечный поцелевой минимум заменяет максимизацию зазора скучиванием логитов, и для гауссовых данных или квадратичной потери доказано, что оптимальное направление — дискриминант Фишера, зависящий только от геометрии данных. [`"In the unregularized case ($\alpha=0$), the per-sample cross-entropy loss $\ell_{\mathrm{CE}}$ is monotonic, and the optimal solution maximizes the margins"`](../papers/2602.12039.the-implicit-bias-of-logit-regularization/original/2602.12039.the-implicit-bias-of-logit-regularization.md#p2-11). *«[В нерегуляризованном случае ($\alpha=0$) поцелевая кросс-энтропийная потеря $\ell_{\mathrm{CE}}$ монотонна, и оптимальное решение максимизирует зазоры](../papers/2602.12039.the-implicit-bias-of-logit-regularization/2602.12039.the-implicit-bias-of-logit-regularization.card.md#p2-11)»*\
Доп. (дискриминант Фишера): [`"If the data is Gaussian, the optimal weight direction is $\hat{\boldsymbol{S}}_{\min}\propto\Sigma^{-1}\mu$"`](../papers/2602.12039.the-implicit-bias-of-logit-regularization/original/2602.12039.the-implicit-bias-of-logit-regularization.md#p3-12) — *«[Если данные гауссовы, оптимальное направление весов — $\hat{\boldsymbol{S}}_{\min}\propto\Sigma^{-1}\mu$](../papers/2602.12039.the-implicit-bias-of-logit-regularization/2602.12039.the-implicit-bias-of-logit-regularization.card.md#p3-12)»*.

###### ref-3-7
**\[3.7\]** 2606.05863 — Tan, Gai & Zhang 2026, «Deciphering Two Training Clocks in Grokking via Deep Linear Network Theory with Conditional ReLU Reduction». Редкая аккуратность в стандартно замалчиваемом месте: намеренно отрицательная лемма о том, что постоянный положительный зазор не даёт равномерного условия Поляка—Лоясевича для кросс-энтропии (гессиан софтмакса вырождается с ростом зазора), — потому быстрые часы подгонки честно висят на допущении роста зазоров, а не на PL-сокращении. [`"margin persistence alone is insufficient to justify geometric decay of the cross-entropy loss"`](../papers/2606.05863.deciphering-two-training-clocks-in-grokking-via-deep-linear-network-theory-with-conditional-relu-reduction/2606.05863.deciphering-two-training-clocks-in-grokking-via-deep-linear-network-theory-with-conditional-relu-reduction.card.md#p8-8). *«[одного сохранения зазора недостаточно для обоснования геометрического убывания перекрёстно-энтропийной потери](../papers/2606.05863.deciphering-two-training-clocks-in-grokking-via-deep-linear-network-theory-with-conditional-relu-reduction/2606.05863.deciphering-two-training-clocks-in-grokking-via-deep-linear-network-theory-with-conditional-relu-reduction.card.md#p8-8)»*.


###### ref-3-8
**\[3.8\]** 2602.08302 — Das, Vedantam, Lakshminarayanan 2026, «Grokking in Linear Models for Logistic Regression». Аппарат Soudry et al. доведён до решаемой модели гроккинга: $\mathbf{w}(t)=\hat{\mathbf{w}}\log t+\rho(t)$ сходится по направлению к max-margin сепаратору рано (фаза 1), а задержку целиком создаёт поздняя опорно-векторная динамика смещения $\frac{db}{dt}=\frac{1}{t}(A_{S}^{+}e^{-b}-A_{S}^{-}e^{b})$ с пределом $b_{\infty}=-\log\delta$, где $\delta=\sqrt{A_{S}^{-}/A_{S}^{+}}$ — перекос опорных векторов. Нюанс: в параметрах модель к обобщающему решению не сходится вовсе ($b_{\infty}\neq 0$) — обобщает только нормированное расстояние $|b|/||\mathbf{w}||$; теорема доказана для экспоненциальной потери, опыты на логистической, а условие «опорные векторы натягивают данные» при $d=1024$ не проверено. [`"For any smooth monotonically decreasing loss function with an exponential tail, and for small learning rate, gradient descent iterates follow"`](../papers/2602.08302.grokking-in-linear-models-for-logistic-regression/original/2602.08302.grokking-in-linear-models-for-logistic-regression.md#p3-17). *«[Для любой гладкой монотонно убывающей функции потерь с экспоненциальным хвостом и при малой скорости обучения итерации градиентного спуска следуют](../papers/2602.08302.grokking-in-linear-models-for-logistic-regression/2602.08302.grokking-in-linear-models-for-logistic-regression.card.md#p3-17)»*\
Доп. (разучивание): [`"The asymmetry between $A_{S}^{+}e^{-b}$ and $A_{S}^{-}e^{b}$ drives $b$ away from the generalizing solution $b=0$."`](../papers/2602.08302.grokking-in-linear-models-for-logistic-regression/original/2602.08302.grokking-in-linear-models-for-logistic-regression.md#p6-3). *«[Асимметрия между $A_{S}^{+}e^{-b}$ и $A_{S}^{-}e^{b}$ уводит $b$ от обобщающего решения $b=0$.](../papers/2602.08302.grokking-in-linear-models-for-logistic-regression/2602.08302.grokking-in-linear-models-for-logistic-regression.card.md#p6-3)»*
## Цитирования
