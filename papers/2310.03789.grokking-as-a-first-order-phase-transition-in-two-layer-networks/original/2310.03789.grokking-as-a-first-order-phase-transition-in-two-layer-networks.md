...
###### p2-5
Our first setting consists of a student Erf-network learning a single index non-linear teacher. The student is trained on a training set of size $n$, $\mathcal{D}=\left\{\bm{x}_{\mu},y\left(\bm{x}_{\mu}\right)\right\}_{\mu=1}^{n}$ with MSE loss. In the following, bold symbol represents a vector. The input vector is $\bm{x}_{\mu}\in\mathbb{R}^{d}$ with iid Gaussian entries of variance **[p. 3]** 1. The target function $y$, is a scalar linear function of $\bm{x}$, with an orthogonal non-linear correction. Specifically, $y$ is given by-

$$
y(\bm{x})=\underbrace{\bm{w}^{*}\cdot\bm{x}}_{H_{1}(\bm{w}^{*}\cdot\bm{x})}+\epsilon\underbrace{\left(\left(\bm{w}^{*}\cdot\bm{x}\right)^{3}-3\left|\bm{w}^{*}\right|^{2}\bm{w}^{*}\cdot\bm{x}\right)}_{H_{3}(\bm{w}^{*}\cdot\bm{x})}. \tag{1}
$$

where $H_{1},H_{3}$ are the first two odd Hermite polynomials, and $\bm{w}^{*}\in\mathbb{R}^{d}$ are the teacher weights. For simplicity we take here the norm of the teacher weights to be 1, but this has no qualitative effect on the theory as long as we require $\left|\bm{w}^{*}\right|\sim\mathcal{O}(1)$. We consider a fully connected non-linear student network with one hidden layer of width $N$ given by

$$
f\left(\bm{x}\right)=\sum_{i=1}^{N}a_{i}\text{erf}\left(\bm{w}_{i}\cdot\bm{x}\right). \tag{2}
$$

where $\bm{w}_{i}\in\mathbb{R}^{d}$ for $i\in[1,N]$ are the students weights. Evidence that this model Groks can be found in App. (D).
...
$$
\mathcal{D} =\left\{\bm{x}_{nm},\bm{y}\left(\bm{x}_{nm}\right)|m,n\in\mathbb{Z}_{P}\right\} \tag{3}
$$

where $\bm{x}_{nm}\in\mathbb{R}^{2P}$, is a vector such that it is zero in all its coordinates except in the coordinates $n$ and $P+m$ where it is 1 (a “two-hot vector”). The target function $\bm{y}\in\mathbb{R}^{P}$ is given by

$$
y_{p}(\bm{x}_{nm}) =\delta_{p,(n+m)\,\text{mod}\,P}-1/P, \tag{4}
$$
...
$$
f_{p}\left(\bm{x}_{nm}\right) =\sum_{i=1}^{N}a_{pi}(\bm{w}_{i}\cdot\bm{x}_{mn})^{2} \tag{5}
$$

where $\bm{w}_{c}\in\mathbb{R}^{2P}$ for $c\in[1,N]$ are the students weights. For brevity, we denote $y_{p}\left(\bm{x}_{mn}\right)=y_{nm}^{p}$, and $f_{p}\left(\bm{x}_{nm}\right)=f_{mn}^{p}$.
...
Our main focus here is the posterior distribution of weights in the input layer ($p(\bm{w}_{i})$) and posterior averaged predictions of the network ($f(\bm{x})$). Such posteriors are generally intractable for deep and/or non-linear networks. Hence, we turn to the approximation of Ref. Seroussi et al. (2023) where a mean-field decoupling between the read-out layer and the input layer is performed. This is exact in the limit of $N\rightarrow\infty$ and vanishing $\sigma_{a}^{2}$ (i.e. mean-field scaling). Following this, the posterior decouples to a product of two probabilities, a Gaussian probability for the read-out layer outputs and a generally non-Gaussian probability for the input layer weights. These two probabilities are coupled via two non-fluctuating quantities. The average kernel induced by the input layer and the discrepancy in predictions. As shown in Seroussi et al. (2023), specifically for a two-layer FCN, the resulting probability further decouples into iid probabilities over each neuron $p(\bm{w}_{i})$. Below, we thus omit the neuron index $i$. The action ($-\log(p(\bm{w}_{i}))$ up to constant normalization factors) for each neuron’s weights is then given by the following form

$$
\mathcal{S}[\bm{w}]=\frac{|\bm{w}|^{2}}{2\sigma_{w}^{2}}-\frac{\sigma_{a}^{2}}{2N}\sum_{\mu,\nu}\overline{\bm{t}}(\bm{x}_{\mu})^{T}\overline{\bm{t}}(\bm{x}_{\nu})\underbrace{\phi(\bm{w}\cdot\bm{x}_{\mu})\phi(\bm{w}\cdot\bm{x}_{\nu})}_{:=\sigma_{a}^{-2}\tilde{Q}_{\mu\nu}}, \tag{**[p. 5]** 7}
$$

where $\phi$ is the activation function and $\bar{\bm{t}}$ is the discrepancy between the averaged network output and the target given by

$$
\overline{\bm{t}}\left(\bm{x}_{\mu}\right)=(\bm{y}\left(\bm{x}_{\mu}\right)-\overline{\bm{f}}\left(\bm{x}_{\mu}\right))/\sigma^{2}. \tag{8}
$$

Notably $\bar{\bm{t}}$ is not given but determined by solving the following two mean-field self-consistency equations

$$
\overline{\bm{f}}=Q\left[Q+\sigma^{2}I_{n}\right]^{-1}\bm{y} \tag{9}
$$

where the kernel $Q$ is defined via

$$
Q_{\mu\nu}=\sigma_{a}^{2}\left\langle\phi\left(\bm{w}\cdot\bm{x}_{\mu}\right)\phi\left(\bm{w}\cdot\bm{x}_{\nu}\right)\right\rangle_{\mathcal{S}[\bm{w}]} \tag{10}
$$

and $\langle...\rangle_{\mathcal{S}[\bm{w}]}$ denotes averaging over $\bm{w}$ with the probability implied by $\mathcal{S}[\bm{w}]$. In this work we use the equivalent kernel (EK) approximation, allowing the sums appearing in eqs. 7,9 to be replaced by integrals. This approximation washes out the generalization phenomena associated with Grokking, while capturing underlying feature learning mechanisms. As demonstrated in App. D, the feature learning effects hold also for finite datasets, in which a generalization gap can be observed. Theoretical corrections due to finite datasets can be made as shown in Cohen et al. (2021); Seroussi et al. (2023); Naveh & Ringel (2021), here we focus on the EK limit for simplicity.

Even if $\bar{t}$ is given, the remaining action is still non-linear. Ref. Seroussi et al. (2023) proceed by performing a variational Gaussian approximation on that action. Here we extend this approximation into a certain variational Gaussian *mixture* approximation. Specifically, we show that as one scales up $d,N,n$ in an appropriate manner, and following some decoupling arguments between, $\mathcal{S}[\bm{w}]$ has the form $d\mathcal{\hat{S}}[\Phi(\bm{w})]$ where $d\gg 1$ and $\mathcal{\hat{S}}[\Phi(\bm{w})]$ has $O(1)$ coefficients and $\Phi(\bm{w})$ is some linear combination of the weights. This allows us to treat the integration underlying $\langle...\rangle_{\mathcal{S}[\bm{w}]}$ within a saddle-point approximation. Notably when more than one Global saddle appears the saddle-point treatment corresponds to $p(\bm{w})$ having a Gaussian mixture form.
...
$$
\sigma^{2}\overline{t}\left(\bm{x}\right)=bH_{1}\left(\bm{x}\right)+cH_{3}\left(\bm{x}\right) \tag{12}
$$
...
**Claim II. One-dimensional posterior weight distribution.** In the same scaling limit, $u\leq u_{c}$ the negative log probability (action in physics terminology) of weights along the teacher direction, decouples from the rest of the $\bm{w}$ modes, and takes the following form

$$
\mathcal{S}[\bm{w}\cdot\bm{w}^{*}] =d\left(\frac{\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{2\sigma_{w}^{2}}-\frac{2n^{2}\sigma_{a}^{2}}{\pi\sigma^{4}dN}\frac{\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left(\sigma_{w}^{2}+\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}\right)}\left(b-\frac{2c\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left(\sigma_{w}^{2}+\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}\right)}\right)^{2}\right) \tag{13}
$$

###### p6-4
Notably, this expression reduces the high-dimensional network posterior into a scalar probability involving only the relevant order parameter ($\Phi=\bm{w}\cdot\bm{w}^{*}$). We further note that all the expressions in the brackets are invariant under $\alpha$ and $\beta$. Heuristically, this will also be the action for a small $\Delta u$ after the phase transition since the resulting corrections to the discrepancy have a parametrically small effect. For further detail see App. A.2.
...
The following symmetries of $S[\bm{w}]$ (which is also a function of $\bar{\bm{t}}$) help us decouple the posterior probability distribution into its Fourier modes and simplify the problem considerably:
...
$$
\mathcal{S}[\hat{\bm{w}}]=P\left[\left(\frac{1}{2}\sum_{k=0}^{P-1}w_{k}w_{-k}+\frac{1}{2}\sum_{k=0}^{P-1}v_{k}v_{-k}\right)-\frac{2\sigma_{a}^{2}a^{2}P^{2}}{N\sigma^{4}}\sum_{k=1}^{P-1}w_{k}w_{-k}v_{k}v_{-k}\right]+\Gamma[\bm{w}] \tag{16}
$$

(see App. B.2) where apart from the non-linear weight-decay term, all different $k$ modes have been decoupled. For simplicity, we next choose, $\Gamma[\bm{w}]=\sum_{k}P\frac{\gamma}{6}\left(\left(w_{k}w_{-k}\right)^{3}+\left(v_{k}v_{-k}\right)^{3}\right)$. Here there are technically two order parameters- $\Phi=w_{k}w_{-k},\Psi=v_{k}v_{-k}$, but from the symmetry of the action, we obtain that the saddles occur only at points where $\Phi=\Psi$. We comment that the analysis of a more natural weight decay terms such as $\sum_{n}w_{n}^{6}+w_{m}^{6}$, using a certain GP mixture ansatz of $p(w_{i})$ as an approximation, we obtained similar qualitative results.
...
The third approximation we carry is the saddle-point approximation or Gaussian-mixture approximation for $p(\bm{w})$. Examining our action

$$
\mathcal{S}\left[\bm{w}\right] =d\left(\frac{\left|\bm{w}\right|^{2}}{2\sigma_{w}^{2}}-\frac{2\chi n_{\text{eff}}^{2}\tilde{\sigma}_{a}^{2}}{\pi dN}\frac{\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left|\bm{w}\right|^{2}}\left(b[n_{\text{eff}}/d]-\frac{2c[\epsilon]\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left|\bm{w}\right|^{2}}\right)^{2}\right) \tag{1}
$$
...
We thus find that the following family of scaling limits controlled by two scaling parameters ($\alpha,\beta\rightarrow\infty$) ensures $S[\bm{w}]$ is exact and its saddle points dominate $\bm{w}$
...
$$
\int d\mu_{z}Q(\bm{x},\bm{z})\phi_{\lambda}(\bm{z}) =\lambda\phi_{\lambda}(\bm{x}) \tag{3}
$$

where $\mu_{z}$ is the dataset measure, which corresponds here to i.i.d. standard Gaussians. Denoting by $g_{\lambda}$ the spectral decomposition of the target function on $\phi_{\lambda}(\bm{x})$, the GPR predictions (Eq. 9) are given by

$$
\bar{f}(\bm{x}) =\sum_{\lambda}\frac{\lambda}{\lambda+\sigma^{2}/n}g_{\lambda}\phi_{\lambda}(\bm{x}) \tag{4}
$$
...
The resulting limit also enables us to identify the two relevant directions in function space, being $H_{1}(\bm{x})$ and $H_{3}(\bm{x})$. Specifically we argue that at large $d$ the operator $Q(\bm{x},\bm{y})$ is block-diagonal within the space spanned by $H_{1}(\bm{x})$ and $H_{3}(\bm{x})$. Since it acts on a target having support only on $H_{1}(\bm{x}),H_{3}(\bm{x})$, this limits $\bar{f}(\bm{x})$ to this space and hence also $\bar{t}(\bm{x})=[y(\bm{x})-\bar{f}(\bm{x})]/\sigma^{2}$. Our claim is shown to be self-consistent in App. A namely, that making this assumption results in $Q$ and $\bar{t}$ which obey the same assumption. Operationally, we thus take the ansatz

$$
\sigma^{2}\overline{t}\left(\bm{x}\right)=bH_{1}\left(\bm{x}\right)+cH_{3}\left(\bm{x}\right) \tag{5}
$$

where $b,c\in\mathbb{R}$ are some unknown constant coefficients. Later, we derive self-consistent equations determining these constants. Given the above ansatz, we turn to the action in Eq. 7, take the continuum approximation where $\sum^{n}_{\mu=1}\to n\int d\mu_{y}$ and find that the integral appearing in the action can be solved exactly- $\overline{t}\left(\bm{x}\right)=bH_{1}\left(\bm{w}^{*}\cdot\bm{x}\right)+cH_{3}\left(\bm{w}^{*}\cdot\bm{x}\right)$, we can compute the action in the continuum limit-

$$
\mathcal{S} \equiv\frac{d\left|\bm{w}\right|^{2}}{2\sigma_{w}^{2}}-\frac{n^{2}\sigma_{a}^{2}}{2N\sigma^{4}}\left(\left(b-3c\left|\bm{w}^{*}\right|^{2}\right)\underbrace{\int d\mu_{x}\left(\bm{w}^{*}\cdot\bm{x}\right)\phi\left(\bm{w}\cdot\bm{x}\right)}_{I_{0}}+c\underbrace{\int d\mu_{x}\left(\bm{w}^{*}\cdot\bm{x}\right)^{3}\phi\left(\bm{w}\cdot\bm{x}\right)}_{I_{1}}\right)^{2} \\
=d\left(\frac{\left|\bm{w}\right|^{2}}{2\sigma_{w}^{2}}-\frac{2n^{2}\sigma_{a}^{2}}{\pi dN\sigma^{4}}\frac{\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left|\bm{w}\right|^{2}}\left(b-\frac{2c\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left|\bm{w}\right|^{2}}\right)^{2}\right) \tag{6}
$$

we the integrals $I_{0},I_{1}$ are computed in App. A.4.

In principle, one could perform saddle point analysis in two variables. A simplification can be made by noting that the dominant fluctuations are in the direction of $\bm{w}^{*}$. **[p. 15]** We are therefore projecting $\bm{w}$ into the subspace of $\bm{w}^{*}$, such that, $\bm{w}=P_{\bm{w}^{\star}}\bm{w}+P_{\bm{w}^{*}}^{\perp}\bm{w}$, with $P_{\bm{w}^{*}}=\frac{1}{|\bm{w}^{*}|^{2}}\bm{w}^{*}(\bm{w}^{*})^{T}$, and $P_{\bm{w}^{*}}=I_{d}-P_{\bm{w}^{*}}^{\perp}$, Denote $\alpha=\bm{w}^{*}\cdot\bm{w}$ and $\alpha_{\perp}=|P_{\bm{w}^{*}}^{\perp}\bm{w}|$ the action is given by

$$
\mathcal{S}[\bm{w}] =\frac{d}{2\sigma_{w}^{2}}\alpha^{2}+\frac{d}{2\sigma_{w}^{2}}\alpha_{\perp}^{2}-\frac{4n^{2}\sigma_{a}^{2}}{\pi 2N\sigma^{4}}\frac{\left|\bm{w}^{*}\right|^{2}\alpha^{2}}{1+2\left(\alpha^{2}+\alpha_{\perp}^{2}\right)}\left(b-\frac{2c\left|\bm{w}^{*}\right|^{2}\alpha^{2}}{1+2\left(\alpha^{2}+\alpha_{\perp}^{2}\right)}\right)^{2}, \tag{7}
$$

By concentration of the norm, we replace $\alpha_{\perp}$ by its average $\sigma_{w}^{2}$ yields the following action-

$$
\tilde{\mathcal{S}}[\bm{w}\cdot\bm{w}^{*}] =d\left(\frac{\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{2\sigma_{w}^{2}}-\frac{2n^{2}\sigma_{a}^{2}}{\pi\sigma^{4}dN}\frac{\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left(\sigma_{w}^{2}+\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}\right)}\left(b-\frac{2c\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left(\sigma_{w}^{2}+\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}\right)}\right)^{2}\right) \tag{8}
$$

### A.3 Solving the equations for $b,c$

Since the kernel does not mix polynomials of higher order, we can write the GPR expression for the mean network output in the function space spanned by the components of the target. Namely, any function in this two-dimensional function space is given by a linear combination of $\hat{H}_{1}(\bm{x})=H_{1}(\bm{x}),\hat{H}_{3}(\bm{x})=\frac{1}{\sqrt{6}}H_{3}(\bm{x})$. The factor of $\frac{1}{\sqrt{6}}$ results from the requirement that the basis of this space will be normal. In this basis, the discrepancy and the target is given by,

$$
\overline{\bm{t}}=\begin{bmatrix}b\\
\sqrt{6}c\end{bmatrix},\bm{y}=\begin{bmatrix}1\\
...
$$
\tilde{Q}=\begin{bmatrix}\int d\mu_{\bm{x}}d\mu_{\bm{z}}\hat{H}_{1}\left(\bm{x}\right)Q\left(\bm{x},\bm{z}\right)\hat{H}_{1}\left(\bm{z}\right)&\int d\mu_{\bm{x}}d\mu_{\bm{z}}\hat{H}_{3}\left(\bm{x}\right)Q\left(\bm{x},\bm{z}\right)\hat{H}_{1}\left(\bm{z}\right)\\
\int d\mu_{\bm{x}}d\mu_{\bm{z}}\hat{H}_{1}\left(\bm{x}\right)Q\left(\bm{x},\bm{z}\right)\hat{H}_{3}\left(\bm{z}\right)&\int d\mu_{\bm{x}}d\mu_{\bm{z}}\hat{H}_{3}\left(\bm{x}\right)Q\left(\bm{x},\bm{z}\right)\hat{H}_{3}\left(\bm{z}\right)\end{bmatrix} \tag{10}
$$

where $\mu_{\bm{x}}$ is the standard Gaussian measure, $Q$ is the mean continuum limit kernel defined as

$$
Q\left(\bm{x},\bm{z}\right)=\sigma_{a}^{2}\int\frac{e^{-\mathcal{S}\left(\bm{w};b,c\right)}}{\mathcal{Z}}\phi\left(\bm{x}\cdot\bm{w}\right)\phi\left(\bm{z}\cdot\bm{w}\right)d\bm{w} \tag{11}
$$

Next, we proceed by calculating the matrix element of $\hat{Q}$. As shown in the integral appendix A.4, we have

$$
\int d\mu_{\bm{x}}\hat{H}_{1}\left(\bm{x}\right)\phi\left(\bm{x}\cdot\bm{w}\right) =\frac{2\left(\bm{w}^{*}\cdot\bm{w}\right)}{\sqrt{\pi}\sqrt{\left(1+2\left|\bm{w}\right|^{2}\right)}} \\
\int d\mu_{\bm{x}}\hat{H}_{3}\left(\bm{x}\right)\phi\left(\bm{x}\cdot\bm{w}\right) =-\frac{4\left(\bm{w}^{*}\cdot\bm{w}\right)^{3}}{\sqrt{6\pi}\sqrt{\left(1+2\left|\bm{w}\right|^{2}\right)^{3}}}
$$

Since the fluctuations in the directions orthogonal to $\bm{w}^{*}$ are small, the quantity $\left|\bm{w}\right|^{2}$ appearing in the denominator can be approximated to $\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}+\left(d-1\right)\frac{\sigma_{w}^{2}}{d}$ as explained in the main text. Since we are taking large values of $d$, then $\frac{d-1}{d}\cong 1$. Additionally, we denote $q=\bm{w}\cdot\bm{w}^{*}$, so that.
...
$$
\begin{bmatrix}b\\
\sqrt{6}c\end{bmatrix}=\overline{\bm{t}}=\left[\tilde{Q}+\frac{\sigma^{2}}{n}I_{2}\right]^{-1}\bm{y}=\left[\tilde{Q}+\frac{\sigma^{2}}{n}I_{2}\right]^{-1}\begin{bmatrix}1\\
...
Here we compute the integral $\big{\langle}\left(\bm{w}^{*}\cdot\bm{x}\right)^{2n+1}\phi\left(\bm{w}\cdot\bm{x}\right)\big{\rangle}_{\bm{x}}$ where $\bm{x}\sim\mathcal{N}\left(0,I_{d}\right)$. This integral appears both in the computation of the action and in the calculation of the matrix elements of $\tilde{Q}$. Denote-

$$
I_{n}\equiv\frac{1}{Z}\int d\bm{x}e^{-\frac{1}{2}\bm{x}^{2}}\phi\left(\bm{w}\cdot\bm{x}\right)\left(\bm{w}^{*}\cdot\bm{x}\right)^{2n+1}. \tag{13}
$$

Where $Z=(2\pi)^{d/2}$ is a normalizing factor. This integral is given by

$$
I_{n} \equiv\frac{1}{Z}\int d\bm{x}e^{-\frac{1}{2}\bm{x}^{2}}\phi\left(\bm{w}\cdot\bm{x}\right)\left(\bm{w}^{*}\cdot\bm{x}\right)^{2n+1} \\
=\frac{2}{\sqrt{\pi}}\frac{\bm{w}\cdot\bm{w}^{*}}{\sqrt{\det\left(\bm{1}+2\bm{w}\bm{w}^{T}\right)}}\left\langle\left(\bm{w}^{*}\cdot\bm{x}\right)^{2n}\right\rangle_{\mathcal{N}\left(0,\left(\bm{1}+2\bm{w}\bm{w}^{T}\right)^{-1}\right)}+2n\left|\bm{w}^{*}\right|^{2}I_{n-1} \\
=\frac{2}{\sqrt{\pi}}\frac{\bm{w}\cdot\bm{w}^{*}}{\sqrt{1+2\left|\bm{w}\right|^{2}}}\left\langle\left(\bm{w}^{*}\cdot\bm{x}\right)^{2n}\right\rangle_{\mathcal{N}\left(0,\left(\bm{1}+2\bm{w}\bm{w}^{T}\right)^{-1}\right)}+2n\left|\bm{w}^{*}\right|^{2}I_{n-1} \tag{14}
$$

By Sherman Morrison’s formula,

$$
\left(\bm{1}+2\bm{w}\bm{w}^{T}\right)^{-1}=\bm{1}-\frac{2\bm{w}\bm{w}^{T}}{1+2\left|\bm{w}\right|^{2}}. \tag{15}
$$

Therefore, a recursive expression for $I_{n}$ can be obtained in terms of $\bm{w}$ and $I_{n-1}$

$$
I_{n} =\frac{1}{Z}\int d\bm{x}e^{-\frac{1}{2}\bm{x}^{2}}\phi\left(\bm{w}\cdot\bm{x}\right)\left(\bm{w}^{*}\cdot\bm{x}\right)^{2n+1} \\
=\frac{2}{\sqrt{\pi}}\frac{\bm{w}\cdot\bm{w}^{*}}{\sqrt{1+2\left|\bm{w}\right|^{2}}}\left\langle\left(\bm{w}^{*}\cdot\bm{x}\right)^{2n}\right\rangle_{\mathcal{N}\left(0,\left(\bm{1}+2\bm{w}\bm{w}^{T}\right)^{-1}\right)}+2n\left|\bm{w}^{*}\right|^{2}I_{n-1}. \tag{16}
$$

The cases $n=0,1$ are the ones used in this work. For $n=0$ the integral is simply-

$$
I_{0}=\frac{2}{\sqrt{\pi}}\frac{\bm{w}\cdot\bm{w}^{*}}{\sqrt{1+2\left|\bm{w}\right|^{2}}}. \tag{17}
$$

For $n=1$ the calculation is slightly more complicated. We have

$$
\left\langle\left(\bm{w}^{*}\cdot\bm{x}\right)^{2}\right\rangle =\sum_{ij}w_{i}^{\prime}w_{j}^{\prime}\left\langle x_{i}x_{j}\right\rangle=\sum_{ij}w_{i}^{\prime}w_{j}^{\prime}\left[\left(\bm{1}+2\bm{w}\bm{w}^{T}\right)^{-1}\right]_{ij} \\
=\sum_{ij}w_{i}^{\prime}w_{j}^{\prime}\left[\delta_{ij}-\frac{2w_{i}w_{j}}{1+2\left|\bm{w}\right|^{2}}\right] \\
=\left|\bm{w}^{*}\right|^{2}-\frac{2\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left|\bm{w}\right|^{2}}. \tag{18}
$$

Which yield,

$$
I_{1} =\frac{2}{\sqrt{\pi}}\frac{\left(\bm{w}\cdot\bm{w}^{*}\right)}{\sqrt{\left(1+2\left|\bm{w}\right|^{2}\right)}}\left(3\left|\bm{w}^{*}\right|^{2}-\frac{2\left(\bm{w}\cdot\bm{w}^{*}\right)^{2}}{1+2\left|\bm{w}\right|^{2}}\right) \tag{19}
$$
...
K_{nm,n^{\prime}m^{\prime}} =\langle(W_{n}+W_{m+P})(W_{n^{\prime}}+W_{m^{\prime}+P})\rangle_{\bm{w}\sim N[0,I_{2P\times 2P}]} \\
...
using $\sum_{n=0}^{P-1}e^{\frac{2\pi i}{P}\left(k+k^{\prime}\right)n}=P\delta_{k,-k}$. The quadratic term in the action (main text equation 3.2) $\left|\bm{w}\right|^{2}$ is given by

$$
\left|\bm{w}\right|^{2}=\sum_{n=0}^{P-1}w_{n}^{2}+\sum_{m=P}^{2P-1}w_{m}^{2}=P\left(\sum_{k=0}^{P-1}w_{k}w_{-k}+\sum_{k=0}^{P-1}v_{k}v_{-k}\right) \tag{28}
$$
...
$$
\mathcal{S}[\hat{\bm{w}}]=P\left[\left(\frac{1}{2}\sum_{k=0}^{P-1}w_{k}w_{-k}+\frac{1}{2}\sum_{k=0}^{P-1}v_{k}v_{-k}\right)-\frac{2\sigma_{a}^{2}a^{2}P^{2}}{N\sigma^{4}}\sum_{k=1}^{P-1}w_{k}w_{-k}v_{k}v_{-k}\right]. \tag{32}
$$
...
$$
\sigma_{a}^{-2}{Q}\bm{y}^{p} =\sum_{t=1}^{P-1}e^{\frac{2\pi i}{P}pt}{Q}\left|t,t\right\rangle=\sum_{t=1}^{P-1}e^{\frac{2\pi i}{P}pt}\sum_{k,k^{\prime}=0}^{P-1}\left\langle\left|w_{k}\right|^{2}\left|v_{k^{\prime}}\right|^{2}\right\rangle\left|k,k^{\prime}\right\rangle\delta_{t,k}\delta_{t,k^{\prime}} \\
=\sum_{k=1}^{P-1}e^{\frac{2\pi i}{P}pk}\left\langle\left|w_{k}\right|^{2}\left|v_{k}\right|^{2}\right\rangle\left|k,k\right\rangle=\sigma_{a}^{-2}\lambda\bm{y}^{P} \tag{37}
$$
