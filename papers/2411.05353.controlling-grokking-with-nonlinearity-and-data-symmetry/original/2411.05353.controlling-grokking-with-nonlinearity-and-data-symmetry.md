...
> **Происхождение конверсии.** Восстановлено из PDF (task-025). У работы (v1,
> единственная версия) нет ни одного из трёх обычных источников: нативный
> `arxiv.org/html/2411.05353` отдаёт 404, ar5iv возвращает страницу аннотации
> (0 маркеров LaTeXML), а `arxiv.org/e-print/2411.05353` — сам PDF
> (`%PDF-1.7`), а не архив с LaTeX, — это подача PDF-only (перепроверено
> 2026-08-27). Разбор поблочный (pymupdf): PDF собран в Microsoft Word for
> Microsoft 365, текстовые блоки идут построчно в порядке потока содержимого;
> абзацы восстановлены по вордовскому отступу первой строки (x≈108 pt против
> x≈72 pt у продолжений) и по буллетам SymbolMT (x≈90 pt). Метки страниц
> точны по построению: они не сопоставлены, а взяты из разбиения PDF;
> колонцифры (15 штук, Calibri 11 pt) не перенесены.
>
> Формулы восстановлены вручную по отрисованным страницам, а не по текстовому
> слою: у Cambria Math в этом файле дроби, пределы сумм и над/подстрочные
> индексы рассыпаны по отдельным блокам с несовпадающими координатами. Все
> семь выключных формул (1)–(7) набраны заново по растру с. 3, 4 и 6;
> строчная математика (модульное сложение, активации, энтропия
> $S=-\sum_{i}w_{i}\,log\,w_{i}$) сверена с растром тех же строк. Авторское
> обозначение активации через символ пустого множества ($\emptyset(x)$,
> U+2205 в текстовом слое) сохранено как напечатано. Иллюстрации — 29 панелей
> рисунков 1–14 — извлечены встроенными файлами прямо из PDF
> (`extract_image`); панели (a)–(f) упорядочены по месту на странице.
> Подписи рисунков 8 и 12 в PDF слиты с соседними абзацными блоками и
> отделены по смыслу; подпись рисунка 12 разорвана переносом страницы.
> Сверка по словам с текстом PDF: расхождений нет (не перенесены только
> колонцифры; выключные формулы вынесены из сверки — они набраны с растра).
