...
**Notations.** We use bold-face letters to denote vectors such as $\bm{x}=(x_{1},\ldots,x_{d})\in\mathbb{R}^{d}$ and use bold-face capital letters to denote matrices such as $\bm{X}=(\bm{x}_{1},\ldots,\bm{x}_{n})^{\top}\in\mathbb{R}^{n\times d}$. **[p. 4]** We denote by $\bm{I}_{d}$ the $d$-dimensional identity matrix. For a vector $\bm{x}$, we denote by $\lVert\bm{x}\rVert_{2}$ its $\ell_{2}$ norm. For a matrix $\bm{X}$, we denote by $\lVert\bm{X}\rVert_{\mathrm{op}}$ and $\lVert\bm{X}\rVert_{F}$ its operator and Frobenius norms, respectively. For a symmetric matrix $\bm{A}$, let $\lambda_{\mathrm{min}}(\bm{A})$ and $\lambda_{\mathrm{min}}^{+}(\bm{A})$ denote its smallest eigenvalue and its smallest positive eigenvalue, respectively. We use $\mathbbm{1}\{\cdot\}$ to denote the indicator function, which equals one when its Boolean input is true and zero otherwise. We use standard asymptotic notations $O(\cdot)$ and $\Omega(\cdot)$ to hide numerical constant factors.

**Linear regression and data generation.** We consider a fundamental regression problem of learning a teacher function $N^{*}(\bm{x})$ where $\bm{x}\in\mathbb{R}^{d}$. The training data $\{(\bm{x}_{i},y_{i})\}_{i=1}^{n}\subseteq\mathbb{R}^{d}\times\mathbb{R}$ are generated according to the following: the input $\bm{x}\sim\mathcal{D}_{\bm{x}}$ follows some marginal distribution $\mathcal{D}_{\bm{x}}$ (which we do not specify), and is exactly labeled by the teacher function, i.e. $y=N^{*}(\bm{x})$. We train a student linear regression model

$$
N(\bm{x};\bm{\theta})=\langle\bm{\theta},\bm{\phi}(\bm{x})\rangle,
$$

where $\bm{\phi}(\bm{x}):\mathbb{R}^{d}\mapsto\mathbb{R}^{m}$ is some fixed feature map and $\bm{\theta}\in\mathbb{R}^{m}$ is the trainable parameters, to learn the teacher. We assume that the teacher is realizable, i.e., $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$.

###### p4-3

**Ridge regression.** In a regression problem, it is natural to train the model by minimizing the empirical mean squared loss $\frac{1}{n}\sum_{i=1}^{n}(N(\bm{x}_{i};\bm{\theta})-N^{*}(\bm{x}_{i}))^{2}$ and evaluate the learning performance on the population squared loss $\mathbb{E}_{\bm{x}}[(N(\bm{x};\bm{\theta})-N^{*}(\bm{x}))^{2}]$. We use ridge regression by appending an $\ell_{2}$-regularization term to the loss function. Let $L_{n}(\bm{\theta};\lambda)$ denote the regularized mean squared training error, let $L_{n}(\bm{\theta})$ denote the unregularized mean squared loss, and let $L_{\lambda}(\bm{\theta})$ denote the $\ell_{2}$ penalty. Our training objective is
...
where $\lambda>0$ is the regularization parameter. We scale the loss objective by a constant factor of $1/2$ for the simplicity of gradient analysis. We optimize Equation (2) using the vanilla Gradient Descent (GD) algorithm, which updates $\bm{\theta}$ via
...
When $N(\bm{x};\bm{\theta})=\langle\bm{\theta},\bm{\phi}(\bm{x})\rangle$ and $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$, our loss objective in Equation (2) can be simplified to
...
To present our results, we introduce the following additional notations. Let $L(\bm{\theta})=\mathbb{E}_{\bm{x}}[(N(\bm{x};\bm{\theta})-N^{*}(\bm{x}))^{2}]$ denote the generalization (population) squared loss. We define the empirical feature map as $\bm{\Phi}=(\bm{\phi}(\bm{x}_{1}),\ldots,\bm{\phi}(\bm{x}_{n}))^{\top}\in\mathbb{R}^{n\times m}$. Let $L=\frac{1}{n}\lVert\bm{\Phi}\rVert_{F}^{2}=\frac{1}{n}\sum_{i=1}^{n}\left\lVert\bm{\phi}(\bm{x}_{i})\right\rVert_{2}^{2}$ denote the (average) norm of the feature vectors. Let $\bm{\Sigma}=\mathbb{E}_{\bm{x}}[\bm{\phi}(\bm{x})\bm{\phi}(\bm{x})^{\top}]\in\mathbb{R}^{m\times m}$ denote the feature population covariance matrix.
...
*Consider a linear regression problem of training a student model $N(\bm{x};\bm{\theta})=\langle\bm{\theta},\bm{\phi}(\bm{x})\rangle$ to learn a realizable teacher $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$, where $\bm{\theta}\in\mathbb{R}^{m}$ is the trainable parameter and $\bm{\theta}^{*}$ is unknown. Suppose that the training is done by optimizing an MSE over $n$ samples using randomly initialized gradient descent at $\bm{\theta}^{(0)}$ with weight decay parameter $\lambda$.*

*Under the initialization $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$ for some $\nu^{2}>0$, and some mild distributional assumptions on the feature map $\bm{\phi}(\bm{x})$, we have with high probability that for a sufficiently large sample size $n$, and a sufficiently large feature space dimensionality $m$, $(t_{2}-t_{1})$ can be lower bounded by an arbitrarily large value by choosing a sufficiently small weight decay $\lambda$.*
...
We start by considering a simple scenario where the target teacher is the zero function. When $N^{*}(\bm{x})=0$, we have
...
*Suppose that the teacher function is $N^{*}(\bm{x})=0$. Consider training a student $N(\bm{x};\bm{\theta})=\langle\bm{\theta},\bm{\phi}(\bm{x})\rangle$ to learn the teacher via optimizing the ridge regression objective in Equation (2) using randomly initialized GD with $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$. Assume that $\eta\leq 2/(L+2\lambda)$. Then for any $t\in\mathbb{N}$, we have*
...
- *With probability at least*$1-2e^{-(m-n)/32}$*,* $L(\bm{\theta}^{(t)})\geq\lambda_{\mathrm{min}}(\bm{\Sigma})\cdot(1-\eta\lambda)^{2t}\cdot\frac{(m-n)\nu^{2}}{2};$
- $\lVert\bm{\theta}^{(t)}\rVert_{2}^{2}\leq(1-\eta\lambda)^{2t}\cdot\lVert\bm{\theta}^{(0)}\rVert_{2}^{2}.$

Studying the zero teacher as a warmup yields the following advantages. First, choosing the zero teacher enables us to build a much simpler generalization guarantee. Clearly, item (iii) above implies that GD eventually converges to the solution $\bm{\theta}^{*}=\bm{0}$. Since $N^{*}(\bm{x})=0$, this yields generalization. Additionally, choosing the zero teacher allows us to derive matching lower (ii) and upper (iii) bounds for the generalization loss in terms of the convergence rate.

###### p5-6

More importantly, Theorem 4.1 reveals that the training and test losses can decrease at different rates, and the potentially considerable discrepancy between these rates causes grokking. Specifically, given the empirical feature map $\bm{\Phi}^{\top}\bm{\Phi}$, a sufficiently small regularization $\lambda$ can only have a negligible effect on the convergence rate of the training MSE. This is because the convergence rate of $(1-\frac{1}{n}\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})-\eta\lambda)^{2t}$ will be dominated by the term $(1-\frac{1}{n}\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi}))^{2t}$ when $\lambda$ is sufficiently small. However, decreasing $\lambda$ can make the generalization convergence rate $(1-\eta\lambda)^{2t}$ arbitrarily slow. In conclusion, Theorem 4.1 establishes a provable instance of grokking whenever $m\gg n$, $\lambda\ll\frac{1}{n}\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})$ and $\lambda\rightarrow 0$.

### 4.2 Grokking with Realizable Teachers

For the most general setting studied in this paper, we consider the realizable case where the ground-truth teacher is $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some unknown $\bm{\theta}^{*}\in\mathbb{R}^{m}$, and the student is $N(\bm{x};\bm{\theta})=\langle\bm{\theta},\bm{\phi}(\bm{x})\rangle$ where $\bm{\theta}\in\mathbb{R}^{m}$ is the vector of trainable parameters. We show that for any teacher, there is a realization of training hyperparameters that results in an arbitrarily long grokking time. Our main result is the following.

##### **Theorem 4.2**(**End-to-end provable grokking for realizable ridge regression**)**.**

###### p5-8

*Assume a realizable teacher function $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$. Assume the boundedness of the feature map, i.e. $\lVert\bm{\phi}(\bm{x})\rVert_{2}\leq b$ for any $\bm{x}$ and some $b>0$. Consider training a student $N(\bm{x};\bm{\theta})=\langle\bm{\theta},\bm{\phi}(\bm{x})\rangle$ to learn the teacher via optimizing the ridge regression objective in Equation (2) using randomly initialized GD with $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$. For all $\epsilon>0$, let $c\geq\epsilon$ be arbitrary. We define*
...
*Moreover, $L(\bm{\theta}^{(t)})\leq\epsilon$ for all sufficiently large $t$.*

###### p6-2

The main intuition behind this discrepancy between $t_{1}$ and $t_{2}$ can be explained as follows: When the student regression model is sufficiently over-parameterized ($m\gg n$), we are solving a high-dimensional regression problem. Therefore, when the weight decay $\lambda$ is small, the GD optimization process only effectively updates the projection of the weight vector $\bm{\theta}_{\parallel}$ onto the data-spanning subspace to fit the training data, while having negligible effect on the component $\bm{\theta}_{\perp}$ in the complementary subspace, which remains close to its initialization. This occurs since the information captured by the data only contributes to the convergence of $\bm{\theta}_{\parallel}$ at a rate of $(1-\frac{1}{n}\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})-\eta\lambda)^{t}$. In contrast, $\bm{\theta}_{\perp}$ converges at a negligible rate of $(1-\eta\lambda)^{t}$ as a consequence of long-term weight decay. Such an imbalanced trajectory prevents timely generalization and leads to harmful overfitting. As a remedy, weight decay plays a crucial role in decreasing the complexity of the model class, which eventually guarantees uniform convergence and thus good generalization.
...
- **Weight decay $\lambda$**: For small $\lambda$, decreasing $\lambda$ increases $t_{2}$ with $t_{2}\propto 1/\lambda$ when other hyperparameters are held fixed, and $\lambda$ has no effect on our bound for $t_{1}$. Therefore, $(t_{2}-t_{1})\rightarrow\infty$ as $\lambda\rightarrow 0$. We remark that for large $\lambda$ (which is not our focus in this work), our upper bound in Equation (6) is not tight in general, as a tight bound should incorporate the term $\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})+\eta\lambda$ in the denominator.
- **Sample size $n$ and feature dimensionality $m$**: It is difficult to deduce clean dependencies that capture the behavior of $t_{1}$ as a function of $n$ and $m$, since there is no known quantitative bound revealing how $\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})$ depends on $m$ and $n$, even when making strong assumptions on the distribution of the features (see Remark 4.3 for a discussion). On a more positive note, we show empirically that the behavior of $t_{1}$ as a function of $n$ and $m$ strongly accords with our theoretical bound in Equation (6). As for $t_{2}$, since we make no assumption on the feature map $\bm{\phi}(\bm{x})$, the quantity $\lambda_{\mathrm{min}}(\bm{\Sigma})$ is not necessarily controlled. Indeed, more explicit dependencies of $t_{2}$ on $n$ and $m$ can be derived by assuming distributional assumptions. For instance, if $\bm{\phi}(\bm{x}_{1}),\ldots,\bm{\phi}(\bm{x}_{n})$ are i.i.d. spherical Gaussian vectors. We present such clean bounds in Equation (8) and show that it strongly matches our experimental results.
...
##### **Remark 4.3**(**on $\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})$ and its dependence on $n$ and $m$**)**.**

###### p6-6

$\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})$*is an empirical quantity which depends on the sampled features. We note that even for simple distributions such as $\bm{\phi}(\bm{x})\sim\mathcal{N}(\bm{0},\frac{1}{m}\bm{I}_{m})$, the behavior of $\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})$ as a function of $n$ and $m$ is not yet fully understood. While there is a known asymptotic result given by the celebrated Marchenko–Pastur law (Marčenko and Pastur, 1967), showing that when $m,n\rightarrow\infty$ and $m/n\rightarrow\gamma$ for some $\gamma\in(0,\infty)$, $\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})\rightarrow\gamma^{-1}(1-\sqrt{\gamma})^{2}$, to the best of our knowledge, there is no quantitative bound revealing how this depends on $m$ and $n$.*
...
*Suppose that $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$, and that $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$. Assume that $\eta<1/(\lambda+\lambda_{\mathrm{max}}(\bm{\Phi}^{\top}\bm{\Phi})/n)$. For any $\epsilon>0$ and $\delta\in(0,1)$, suppose that*
...
*Then, with probability at least $1-\delta$, we have $L_{n}(\bm{\theta}^{(t)})\leq\epsilon$ for all*
...
The above assumption that $m=\Omega(\lVert\bm{\theta}^{*}\rVert_{2}^{2}/\nu^{2})$ implies that the student is initialized to have a large norm with respect to the teacher’s norm. Since we aim to upper bound $L_{n}(\bm{\theta}^{(t)})$ and not $L_{n}(\bm{\theta}^{(t)};\lambda)$, the assumed upper bound on $\lambda$ is for technical purposes, and it does not limit our results since small values of $\lambda$ facilitate grokking.

##### **Theorem 4.5**(**Poor generalization when overfitting**)**.**

*Suppose that $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$, that $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$, and further assume that $\eta<1/\lambda$. For any $c>0$ and $\delta\in(0,1)$, suppose that*

$$
m=n+\Omega\left(\max\left\{\log\left(\frac{1}{\delta}\right),\frac{\lVert\bm{\theta}^{*}\rVert_{2}^{2}}{\nu^{2}},\frac{c}{\lambda_{\mathrm{min}}(\bm{\Sigma})\nu^{2}}\right\}\right).
$$

*Then, with probability at least $1-\delta$, we have $L(\bm{\theta}^{(t)})\geq c$ for all*
...
*Suppose that $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$. Assume that $\lVert\bm{\phi}(\bm{x})\rVert_{2}\leq b$ for any $\bm{x}$ for some $b>0$. Let $\bm{\theta}^{*}_{\lambda}=\operatorname*{arg\,min}_{\bm{\theta}}\left\{L_{n}(\bm{\theta};\lambda)\right\}$. For any $\epsilon>0$ and $\delta\in(0,1)$, suppose that*
...
*Then, if $\eta\leq 1/(\lambda+b^{2})$, GD converges to $\bm{\theta}^{*}_{\lambda}$ satisfying $L(\bm{\theta}^{*}_{\lambda})\leq 2L_{n}(\bm{\theta}^{*}_{\lambda})+\epsilon$, with probability at least $1-\delta$.*

###### p7-5

The lower bound on the sample size $n=\Omega(b^{4}\lVert\bm{\theta}^{*}\rVert_{2}^{4}\epsilon^{-2})$ stems from a standard uniform convergence argument based on Rademacher complexity.
...
We empirically verify our quantitative bounds on how the model’s hyperparameters affect grokking in realizable ridge regression. Specifically, we train a student linear regression model using gradient descent with weight decay, to learn a realizable teacher with unit norm, i.e. $\lVert\bm{\theta}^{*}\rVert_{2}=1$. We simply choose the feature map to be the identity function, i.e. $\bm{\phi}(\bm{x})=\bm{x}\in\mathbb{R}^{m}$. We use $\nu^{2}$ to denote the initialization scale of weights in line with our theory.

###### p7-8

As discussed in Section 4, $\lambda_{\mathrm{min}}(\bm{\Sigma})$ cannot be controlled without making additional assumptions on $\bm{\phi}(\bm{x})$. Consequentially, by introducing additional distributional assumptions, we are able to provide more explicit bounds for $t_{1}$ and $t_{2}$ based on Theorem 4.2. Specifically, we assume that $\bm{\phi}(\bm{x}_{1}),\ldots,\bm{\phi}(\bm{x}_{n})$ are drawn i.i.d. from a Gaussian distribution $\mathcal{N}(\bm{0},\frac{1}{m}\bm{I}_{m})$. In such a case, if $\eta\lambda\leq 0.01$, a simple calculation (see Remark A.14 for details) yields that
...
In this subsection, we train a two-layer random ReLU features network to learn a single ReLU neuron teacher network with unit norm weights, i.e. $\bm{x}\mapsto\sigma(\langle\bm{w}^{*},\bm{x}\rangle)$ with $\lVert\bm{w}^{*}\rVert_{2}=1$, where $\sigma(\cdot)=\max\{0,\cdot\}$ is the ReLU activation. A two-layer ReLU neural network is in the form of $N(\bm{x};\bm{\theta})=\sum_{j=1}^{m}a_{j}\sigma(\langle\bm{w}_{j},\bm{x}\rangle)$ for all $\bm{x}\in\mathbb{R}^{d}$. For random-features neural networks, we only optimize the output layer weights $\bm{\theta}=\bm{a}$, and the hidden layer weights are initialized and then fixed during training. Clearly, this random feature model is a special case of linear regression with feature map $\bm{\phi}(\bm{x})=(\bm{\phi}_{1}(\bm{x}),\ldots,\bm{\phi}_{m}(\bm{x}))$ where $\bm{\phi}_{j}(\bm{x})=\sigma(\langle\bm{w}_{j},\bm{x}\rangle)$ for all $j\in[m]$. We use the following initialization scheme $a_{j}^{(0)}\sim\mathcal{N}(0,1),\bm{w}_{j}\sim\mathcal{N}(\bm{0},\frac{\nu^{2}}{dm}\bm{I}_{d})$ for each $j\in[m]$.
...
*Assume that $0<\eta\leq 2/(L+2\lambda)$ and $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$. Then for any $t\in\mathbb{N}$, we have*

- $L_{n}(\bm{\theta}^{(t)})\leq\frac{L}{2}\cdot(1-\frac{1}{n}\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})-\eta\lambda)^{2t}\cdot\lVert\bm{\theta}^{(0)}\rVert_{2}^{2};$
- *w.p. at least*$1-2e^{-(m-n)/32}$*,*$L(\bm{\theta}^{(t)})\geq\lambda_{\mathrm{min}}(\bm{\Sigma})\cdot\left(1-\eta\lambda\right)^{2t}\cdot\frac{(m-n)\nu^{2}}{2};$
...
*Let $\bm{\Gamma}:=\frac{1}{n}\bm{\Phi}^{\top}\bm{\Phi}+\lambda\bm{I}_{m}$ and let $\lambda_{\mathrm{min}}(\bm{\Gamma})$ denote its smallest eigenvalue. Following a standard argument for showing convergence for ridge regression, we can easily prove $L_{n}(\bm{\theta}^{(t)})\leq L(1-\eta\lambda_{\mathrm{min}}(\bm{\Gamma}))^{2t}\lVert\bm{\theta}^{(0)}\rVert_{2}^{2}$. To show grokking, we need to prove that the convergence of the training error is faster than the generalization error. However, when the feature map $\bm{\Phi}$ is not full rank, $\lambda_{\mathrm{min}}(\bm{\Gamma})=\lambda$, making this rate match the lower bound in Theorem A.4 below, and thus does not suffice to prove grokking. Therefore, a more tight bound on the convergence rate is required here.*

##### Proof of Theorem A.2.

For any vector $\bm{\theta}\in\mathbb{R}^{m}$, we write the following unique decomposition $\bm{\theta}=\bm{\theta}_{\parallel}+\bm{\theta}_{\perp}$ where $\bm{\theta}_{\parallel}$ is in the row space of $\bm{\Phi}$ and $\bm{\theta}_{\perp}$ is orthogonal to the row space of $\bm{\Phi}$ (or equivalently, $\bm{\theta}_{\perp}$ is in the null space of $\bm{\Phi}$). We can express the training loss (at $\bm{\theta}$)
...
as a function of $\bm{\theta}_{\parallel}$. Note that
...
Let $\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})$ denote the smallest non-zero eigenvalue of the matrix $\bm{\Phi}^{\top}\bm{\Phi}$. It follows that
...
Since $\bm{\theta}^{*}=\bm{0}$ and $L_{n}(\bm{\theta}^{*})=0$, we have from Equations (A) and (11) that
...
Note that the convergence rate of $(1-\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})/n-\eta\lambda/n)^{2t}$ is always faster than $(1-\eta\lambda/n)^{2t}$. In particular, a small weight decay $\lambda$ will amplify grokking. The proof is done! ∎

##### **Theorem A.4**(**Overfitting**)**.**

*Assume that $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$. Then, for all $t\in\mathbb{N}$, we have*
...
Recall that $\bm{\Phi}:=(\bm{\phi}(\bm{x}_{1}),\ldots,\bm{\phi}(\bm{x}_{n}))^{\top}$ and $\bm{\theta}^{(t+1)}=\bm{\theta}^{(t)}-(\eta/n)\bm{\Phi}^{\top}\bm{\Phi}\bm{\theta}^{(t)}-\eta\lambda\bm{\theta}^{(t)}$. For any $\bm{\theta}\in\mathbb{R}^{m}$, we inherit the notations $\bm{\theta}=\bm{\theta}_{\parallel}+\bm{\theta}_{\perp}$ used in the proof of Theorem A.2. It holds that
...
Note that $\bm{\theta}_{\perp}^{(0)}$ lies in a subspace of dimensionality $k$, which is at least $m-n$. Let $\bm{e}_{1},\ldots,\bm{e}_{k}$ represent an orthogonal basis for the subspace, we have

$$
\lVert\bm{\theta}_{\perp}^{(0)}\rVert_{2}^{2}=\sum_{j=1}^{k}\left(\langle\bm{\theta}^{(0)},\bm{e}_{j}\rangle\right)^{2}.
$$

Since $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{d})$, we have that $\langle\bm{\theta}^{(0)},\bm{e}_{j}\rangle,j\in[k]$ are independent $\mathcal{N}(0,\nu^{2})$ random variables. By standard concentration inequality for Chi-squared distribution (c.f. Dudeja and Hsu, 2018, Fact 5), we have that with probability at least $1-2\exp(-k/32)$,
...
This implies that GD eventually reaches $\bm{\theta}^{*}=\bm{0}$ and yields generalization. ∎

**[p. 15]**

##### **Theorem A.7**(**Theorem 4.2 restated**)**.**

*Assume a realizable teacher function $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$. Assume the boundedness of the feature map, i.e. $\lVert\bm{\phi}(\bm{x})\rVert_{2}\leq b$ for any $\bm{x}$ and some constant $b>0$. Randomly initialize a student linear regression model $N(\bm{x};\bm{\theta}^{(0)})=\langle\bm{\theta}^{(0)},\bm{\phi}(\bm{x})\rangle$ with $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$. Consider training the student to learn the teacher using gradient descent on the ridge regression objective in Equation (2). Let $c\geq\epsilon$ be a constant. For any $\epsilon>0$, define*
...
*Moreover, $L(\bm{\theta}^{(t)})\leq\epsilon$ for sufficiently large $t$.*
...
We use the following Taylor expansion $\ln(1-x)=-x-\frac{x^{2}}{2}-\frac{x^{3}}{3}-\cdots$. For a sufficiently small $\lambda>0$, it implies that $\ln(1-\eta\lambda)\geq-2\eta\lambda$ and also $\ln(1-\frac{\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})}{n})\leq-\frac{\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})}{n}$. Now, we can further write
...
The bounds on the grokking time are proved via bounding $L\leq b^{2}$. Finally, we explain the generalization guarantee. Let $\bm{\theta}^{*}_{\lambda}=\operatorname*{arg\,min}_{\bm{\theta}}\left\{L_{n}(\bm{\theta};\lambda)\right\}$. Fix some $t>0$, we have by triangle inequality that
...
Since $\bm{\theta}^{(t)}$ converges to $\bm{\theta}^{*}_{\lambda}$ (as we show in the proof of Theorem A.10), we have $L(\bm{\theta}^{(t)})-L(\bm{\theta}^{*}_{\lambda})\leq\epsilon/4$ for some sufficiently large $t>0$. Applying Theorem A.10 with $\epsilon/4$ yields that $L(\bm{\theta}^{*}_{\lambda})-2L_{n}(\bm{\theta}^{*}_{\lambda})\leq\epsilon/4$, with probability at least $1-\delta$. Also, we can have $2L_{n}(\bm{\theta}^{*}_{\lambda};\lambda)\leq\epsilon/2$ with a sufficiently small $\lambda$. Note that we proved the theorem with probability $1-3\delta$ here. This does not matter since in the theorem assumptions $\delta$ only appears in asymptotic notations which ignore constant factors. ∎

##### **Theorem A.8**(**Theorem 4.4 restated**)**.**

*Suppose that $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$. Assume that $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$. For any $\epsilon>0$ and $\delta\in(0,1)$, if*
...
*then, if $\eta<1/(\lambda+\lambda_{\mathrm{max}}(\bm{\Phi}^{\top}\bm{\Phi})/n)$, when*

$$
t\geq\frac{1}{2}\log_{1-\frac{\eta\lambda_{\mathrm{min}}^{+}(\bm{\Phi}^{\top}\bm{\Phi})}{n}-\eta\lambda}\left(\frac{\epsilon}{6L\lVert\bm{\theta}^{(0)}\rVert_{2}^{2}}\right),
$$

*we have $L_{n}(\bm{\theta}^{(t)})\leq\epsilon$ with probability at least $1-\delta$.*

##### Proof of Theorem A.8.

Let $\bm{\theta}^{*}_{\lambda}$ denote the global minimum of the regularized objective. We consider the following decomposition of the unregularized squared loss objective:

$$
L_{n}(\bm{\theta}^{(t)})=L_{n}(\bm{\theta}^{(t)})-L_{n}(\bm{\theta}^{*}_{\lambda})+L_{n}(\bm{\theta}^{*}_{\lambda}).
$$

Since $\bm{\theta}^{*}_{\lambda}$ is the global minimum, we have
...
Let $\bm{u}^{(t)}:=\bm{\theta}^{(t)}-\bm{\theta}^{*}_{\lambda}$ for any $t\geq 0$. The above implies that
...
For any vector $\bm{u}\in\mathbb{R}^{m}$, we can write a unique decomposition $\bm{u}=\bm{u}_{\parallel}+\bm{u}_{\perp}$ where $\bm{u}_{\parallel}$ is in the row space of $\bm{\Phi}$ and $\bm{u}_{\perp}$ is orthogonal to the row space of $\bm{\Phi}$ (or equivalently, $\bm{u}_{\perp}$ is in the null space of $\bm{\Phi}$). Then, we can further write the gradient descent along with the orthogonal decomposition:
...
We can write $L_{n}(\bm{\theta}^{(t)})-L_{n}(\bm{\theta}^{*}_{\lambda})$ as follow:
...
Recall that $\frac{1}{n}\sum_{i=1}^{n}\langle\bm{\theta}^{*}_{\lambda},\bm{\phi}(\bm{x}_{i})\rangle\bm{\phi}(\bm{x}_{i})-\frac{1}{n}\sum_{i=1}^{n}\langle\bm{\theta}^{*},\bm{\phi}(\bm{x}_{i})\rangle\bm{\phi}(\bm{x}_{i})+\lambda\bm{\theta}^{*}_{\lambda}=\bm{0}$, which implies that $\frac{1}{n}\sum_{i=1}^{n}\langle\bm{\theta}^{*}_{\lambda,\parallel}-\bm{\theta}^{*}_{\parallel},\bm{\phi}(\bm{x}_{i})\rangle\bm{\phi}(\bm{x}_{i})+\lambda\bm{\theta}^{*}_{\lambda,\parallel}=\bm{0}$ and thus
...
It remains to bound $\lVert\bm{\theta}^{*}_{\lambda,\parallel}\rVert_{2}$ and $L_{n}(\bm{\theta}^{*}_{\lambda})$. Since $\bm{\theta}^{*}_{\lambda}$ is the global minimum of the regularized objective and $L_{n}(\bm{\theta}^{*})=0$, we have
...
It follows immediately that $\lVert\bm{\theta}^{*}_{\lambda,\parallel}\rVert_{2}\leq\lVert\bm{\theta}^{*}_{\lambda}\rVert_{2}\leq\lVert\bm{\theta}^{*}\rVert_{2}$ and $L_{n}(\bm{\theta}^{*}_{\lambda})\leq\frac{\lambda}{2}\lVert\bm{\theta}^{*}\rVert_{2}^{2}$. Moreover, note that
...
where the last step holds if $\lVert\bm{\theta}^{*}\rVert_{2}\leq\lVert\bm{\theta}^{(0)}\rVert_{2}$. Since $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$, when $m=\Omega(\log(1/\delta))$, we have from Lemma C.1 that $\lVert\bm{\theta}^{(0)}\rVert_{2}^{2}=\Omega(m\nu^{2})$ with probability at least $1-\delta$. In particular, when

$$
m=\Omega\left(\max\left\{\log\left(\frac{1}{\delta}\right),\frac{\lVert\bm{\theta}^{*}\rVert_{2}^{2}}{\nu^{2}}\right\}\right),
$$

we have $\lVert\bm{\theta}^{*}\rVert_{2}\leq\lVert\bm{\theta}^{(0)}\rVert_{2}$ with probability at least $1-\delta$. Altogether, we have for any $t\in\mathbb{N}$,
...
*Suppose that $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$. Assume that $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$. For any constant $c>0$ and $\delta\in(0,1)$, if*
...
*we have $L(\bm{\theta}^{(t)})\geq c$ with probability at least $1-\delta$.*

##### Proof of Theorem A.9.

Let $\bm{\Sigma}:=\mathbb{E}_{\bm{x}}[\bm{\phi}(\bm{x})\bm{\phi}(\bm{x})^{\top}]$. Since $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$, we have
...
Using the same notation $\bm{\theta}=\bm{\theta}_{\parallel}+\bm{\theta}_{\perp}$ for any $\bm{\theta}\in\mathbb{R}^{m}$ where $\bm{\theta}_{\perp}$ is orthogonal to the row space of $\bm{\Phi}$, we can write
...
When $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{d})$, following the same analysis in the proof of Theorem A.4, we get
...
suffice to guarantee that $L(\bm{\theta}^{(0)})\geq c$. Then, when
...
*Suppose that $N^{*}(\bm{x})=\langle\bm{\theta}^{*},\bm{\phi}(\bm{x})\rangle$ for some $\bm{\theta}^{*}\in\mathbb{R}^{m}$. Assume that $\lVert\bm{\phi}(\bm{x})\rVert_{2}\leq b,\forall\bm{x}$ for some $b>0$. For any $\epsilon>0$ and $\delta\in(0,1)$, if*
...
Let $S_{n}=\{(\bm{x}_{i},y_{i})\}_{i=1}^{n}$ be a dataset and let $\mathcal{H}$ be a function class. The (empirical) Rademacher complexity of $\mathcal{H}$ with respect to $S_{n}$ is defined as follow
...
The (empirical) Rademacher complexity of $\mathcal{H}_{\bm{\theta}}(B)$ can be bounded as shown in the following lemma.

##### **Lemma A.12****.**

*We have $\mathrm{Rad}_{S_{n}}(\mathcal{H}_{\bm{\theta}}(B))\leq B\sqrt{L/n}$ where $L:=\frac{1}{n}\sum_{i=1}^{n}\lVert\bm{\phi}(\bm{x}_{i})\rVert_{2}^{2}$.*
...
*Let $\mathcal{H}_{\bm{\theta}}$ be a function class and $S_{n}=\{(\bm{x}_{i},y_{i})\}_{i=1}^{n}$ be a dataset independently selected according to some probability measure $P$. For any $N(\bm{x};\bm{\theta})\in\mathcal{H}_{\bm{\theta}}$, let*
...
i.e., the loss objective is smooth. Hence, we know that GD converges to the global minimum $\bm{\theta}^{*}_{\lambda}$ of the regularized loss objective if using a small enough step size $\eta<2/(b^{2}+\lambda)$. Moreover, since $L_{n}(\bm{\theta}^{*}_{\lambda};\lambda)\leq L_{n}(\bm{\theta}^{*};\lambda)$ and $L_{n}(\bm{\theta}^{*})=0$, we have $\frac{\lambda}{2}\lVert\bm{\theta}^{*}_{\lambda}\rVert_{2}^{2}\leq L_{n}(\bm{\theta}^{*}_{\lambda})+\frac{\lambda}{2}\lVert\bm{\theta}^{*}_{\lambda}\rVert_{2}^{2}=\frac{\lambda}{2}\lVert\bm{\theta}^{*}\rVert_{2}^{2}$, i.e., $\lVert\bm{\theta}^{*}_{\lambda}\rVert_{2}\leq\lVert\bm{\theta}^{*}\rVert_{2}$. Now, we know $N(\bm{x}_{i};\bm{\theta}^{*}_{\lambda})\in\mathcal{H}_{\bm{\theta}}(\lVert\bm{\theta}^{*}\rVert_{2})$.

Note that, in order to apply Lemma A.13 to $\mathcal{H}_{\bm{\theta}}(\lVert\bm{\theta}^{*}\rVert_{2})$, we need to argue the Lipschitzness of the loss function. While the squared loss is not Lipschitz globally, it is indeed Lipschitz on a bounded domain. Since we assume that $\lVert\bm{\phi}(\bm{x})\rVert_{2}\leq b$ for some universal constant $b>0$, we have for any $N(\bm{x};\bm{\theta})\in\mathcal{H}_{\bm{\theta}}(\lVert\bm{\theta}^{*}\rVert_{2})$,
...
Hence, the squared loss is $4b\lVert\bm{\theta}^{*}\rVert_{2}$-Lipschitz in the first argument for $\mathcal{H}_{\bm{\theta}}(\lVert\bm{\theta}^{*}\rVert_{2})$. Next, we argue the boundedness of the squared loss objective. This is clear since for any $N(\bm{x};\bm{\theta})\in\mathcal{H}_{\bm{\theta}}(\lVert\bm{\theta}^{*}\rVert_{2})$, we have

$$
|\ell(N(\bm{x};\bm{\theta}),y)|=(N(\bm{x};\bm{\theta})-N(\bm{x};\bm{\theta}^{*}))^{2}\leq 4b^{2}\lVert\bm{\theta}^{*}\rVert_{2}^{2}.
$$

Additionally, we also have $L=\frac{1}{n}\sum_{i=1}^{n}\lVert\bm{\phi}(\bm{x}_{i})\rVert_{2}^{2}\leq b^{2}$.
...
Therefore, with probability at least $1-\delta$, we have $L(\bm{\theta}^{*}_{\lambda})\leq 2L_{n}(\bm{\theta}^{*}_{\lambda})+\epsilon$. ∎

**[p. 22]**

##### **Remark A.14****.**

*We discuss how to derive our bounds in Equation (8) when assuming specific distributions over features. Specifically, under the setting and notations of Theorem 4.2, we assume further that $\bm{\phi}(\bm{x}_{1}),\ldots,\bm{\phi}(\bm{x}_{n})$ are drawn i.i.d. from $\mathcal{N}(\bm{0},\frac{1}{m}\bm{I}_{m})$. For any $\epsilon>0$, we also assume that $m=\Omega(\lVert\bm{\theta}^{*}\rVert_{2}^{2}/\epsilon)$. The bounds then follow directly from applying Theorem 4.2. Since $\bm{\phi}(\bm{x}_{1}),\ldots,\bm{\phi}(\bm{x}_{n})$ are i.i.d. $\mathcal{N}(\bm{0},\frac{1}{m}\bm{I}_{m})$ random variables, we can have $b^{2}=3/2$ with high probability and $\lambda_{\mathrm{min}}(\bm{\Sigma})=1/m$. Moreover, since $\bm{\theta}^{(0)}\sim\mathcal{N}(\bm{0},\nu^{2}\bm{I}_{m})$, we have $m\nu^{2}/2\leq\lVert\bm{\theta}^{(0)}\rVert_{2}^{2}\leq 3m\nu^{2}/2$ with high probability. For $t_{1}$, we have*
...
*For $t_{2}$, note that according to the assumption, we have $c=\epsilon$ and $\epsilon>\lambda_{\mathrm{min}}(\bm{\Sigma})\lVert\bm{\theta}^{*}\rVert_{2}^{2}=\lVert\bm{\theta}^{*}\rVert_{2}^{2}/m$ and thus*
...
*Let $\bm{w}\in\mathbb{R}^{d}$ such that $\bm{w}\sim\mathcal{N}(\bm{0},\sigma^{2}\bm{I}_{d})$ for some $\sigma^{2}>0$. For all $t\in(0,1)$,*
