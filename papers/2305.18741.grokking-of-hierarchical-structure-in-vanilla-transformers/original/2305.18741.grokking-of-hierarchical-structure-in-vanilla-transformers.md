...
$$
\phi_{\text{proj}},T_{\text{proj}}\triangleq\argmin_{\phi,T}\mathcal{L}(f,g_{\phi},T), \tag{1}
$$
...
$$
\mathrm{SCI}{}(S,T)\triangleq\sum_{s\in T}d({\bm{v}}^{S}_{p},\tilde{{\bm{v}}}_{p}) \tag{3}
$$
...
$$
t_{\text{parseval}}{}\triangleq\textsc{PARSEVAL}(\widehat{T}_{\text{proj}},T_{g},\mathcal{D}). \tag{4}
$$
