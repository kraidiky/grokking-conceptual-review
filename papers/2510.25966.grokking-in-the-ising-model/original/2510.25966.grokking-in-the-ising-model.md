...
> **Происхождение конверсии.** Восстановлено из PDF. У работы (v2, актуальная
> версия на 2026-08-14) нет ни одного из трёх обычных источников: нативный
> `arxiv.org/html/2510.25966` отдаёт 404, ar5iv перенаправляет на страницу
> аннотации, а `arxiv.org/e-print/2510.25966` возвращает сам PDF (`%PDF-1.6`),
> а не архив с LaTeX, — это подача PDF-only. Разбор поблочный (pymupdf): PDF
> собран в Word (Acrobat PDFMaker 25), где порядок блоков в потоке содержимого
> и есть порядок чтения, поэтому он взят из самого файла, а не угадан
> геометрической сортировкой — она здесь ломается, потому что у плавающих
> объектов Word рамки блоков не совпадают с их местом на странице (на с. 6 и 11
> подпись рисунка стоит по координатам «внутри» соседнего абзаца). Границы
> абзацев взяты по авторскому разделителю: Word ставит между абзацами строку из
> одного пробела. Метки страниц точны по построению: они не сопоставлены, а
> взяты из разбиения PDF.
>
> Формулы восстановлены вручную по отрисованным страницам, а не по текстовому
> слою: у шрифта Cambria Math в этом файле два дефекта. Во-первых, каждый
> символ из блока Mathematical Alphanumeric Symbols выводится дважды (`𝐸𝐸`
> вместо `E`). Во-вторых — и это опаснее — внутри одного спана таблица
> ToUnicode отображает все глифы на первый: `⟨ij⟩` извлекается как `⟨ii⟩`,
> подстрочное `max` — как `mmm`, `second` — как `ssssss`. Поэтому каждое
> многобуквенное выражение (в том числе $E = -\sum_{\langle ij\rangle} s_i s_j$
> и $\overline{\Delta P} = \overline{P_{max} - P_{second}}$) прочитано с
> растра страницы. Обе выключные формулы, (1) и (2), набраны заново по
> отрисованной с. 9; у формулы (2) предел суммирования $\omega \in \Omega$
> лежит отдельным блоком в 8.5 pt и в тексте не повторяется.
>
> Обе таблицы приходят из PDF плоской строкой без разделителей строк и колонок
> и пересобраны вручную по отрисованным с. 3 и 4. Иллюстрации — 31 панель
> рисунков 1-20 — извлечены встроенными PNG прямо из PDF (`extract_image`),
> одинаковых файлов нет; панели (a)/(b) упорядочены по месту на странице
> (у рис. 19 половинки различаются по вертикали на 1 pt, поэтому строки
> сгруппированы с допуском). Сверка по словам с текстом PDF: расхождений нет
> (не перенесены только 17 колонцифр; тела таблиц и два числа, набранных
> Cambria Math, вынесены из сверки — они на месте в тексте). В PDF осталась
> авторская заметка-стикер на с. 5 («Crop this», Karolina Hutchison,
> 2026-01-23) — она не часть текста статьи и не перенесена.
