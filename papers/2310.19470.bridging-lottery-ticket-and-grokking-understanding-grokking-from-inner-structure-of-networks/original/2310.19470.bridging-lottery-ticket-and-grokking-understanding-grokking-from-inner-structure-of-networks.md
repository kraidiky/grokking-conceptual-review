...
Among those, one of the dominant explanations regarding how the network changes during the process of grokking are the simplicity of the generalization solution, particularly focusing on the weight norms of network parameters $\|\bm{\theta}\|_{2}$. For example, the original paper (Power et al., 2022) posited that weight decay plays a pivotal role in grokking, i.e., test accuracy will not increase without weight decay. Liu et al. (2023a) analyzed the loss landscapes of train and test dataset, verifying that grokking occurs by entering the generalization zone defined by L2 norm, with models having large initial values $\bm{\theta}_{0}$. More recently, Varma et al. (2023) demonstrated that the generalization solution could produce higher logits with smaller weight norms. In this paper, we examine the changes in the network’s structure and demonstrate that the network is not simply decreasing its overall weight norms but searching for good structures within itself.
...
1. Initialize dense network $f_{\bm{\theta_{0}}}$ and train the network for $t$ epochs to obtain the weights $\bm{\theta_{t}}$
2. Perform $k\%$ pruning on the trained network based on absolute values $|\bm{\theta_{t}}|$. This process, known as magnitude pruning, yields a mask $\bm{m}_{t}^{k}\in\{0,1\}^{|\bm{\theta_{t}}|}$.
3. Reset the weights of the network to their initial values. $\bm{\theta_{0}}$ and get a subnetwork $f_{\bm{\theta_{0}}\odot\bm{m}_{t}^{k}}$, representing *lottery ticket*. Train the subnetwork for $t^{\prime}$ epochs and obtain $f_{\bm{\theta^{\prime}_{t^{\prime}}}\odot\bm{m}_{t}^{k}}$.

After the discovery of the lottery tickets, Ramanujan et al. (2020) show that there exist **strong lottery tickets**, which achieve good performance without weight update. They use the **edge-popup** algorithm (Ramanujan et al., 2020) to selects subnetworks based on a score $\bm{s}~{}(\bm{s}\in\mathbb{R}^{|\bm{\theta_{0}}|})$. In other words, when pruning a certain proportion $k$ of weights from the given weights $\bm{\theta}_{0}$, the model predicts using edges with the top $(1-k)$ scores in a forward pass. For a detailed description of edge-popup, refer to Appendix A. In section 5, we use the edge-popup algorithm to check if pruning can be worked as a force to accelerate the grokking process.
...
Following Liu et al. (2022), we design the MLP as follows. Firstly, we map the one-hot encoding of $\bm{a}$, $\bm{b}$ with the embedding weights $W_{\text{emb}}$: $\bm{E}_{a}=W_{\text{emb}}\bm{a},\bm{E}_{b}=W_{\text{emb}}\bm{b}$. We then feed the embeddings $\bm{E}_{a}$ and $\bm{E}_{b}$ into the MLP as follows:

$$
\mathrm{softmax}(\sigma((\bm{E}_{a}+\bm{E}_{b})W_{\text{in}})W_{\text{out}}W_{\text{unemb}}) \tag{1}
$$
...
Following Nanda et al. (2023), we used the AdamW optimizer (Loshchilov & Hutter, 2019) with a learning rate $10^{-3}$, the weighting of weight decay $\alpha=1.0$, $\beta_{1}=0.9$, and $\beta_{2}=0.98$. We initialize weights as $\bm{\theta}_{0}\sim\mathcal{N}(0,\kappa/\sqrt{d_{\text{in}}})$, where $d_{\text{in}}$ represents the dimensionality of the layer preceding each weight. If nothing is specified, assume $\kappa=1$. Let us assume we have training datasets $\mathbb{S}_{\text{train}}$ and test datasets $\mathbb{S}_{\text{test}}$, and train a neural network $f(\bm{x};\bm{\theta})$ where $\bm{x}$ is an input and $\bm{\theta}$ represents weight parameters of the networks. **[p. 5]** Specifically, the network is trained using AdamW over a cross-entropy loss and weight decay (L2 norm of weights $\|\bm{\theta}\|_{2}$):

$$
\underset{{\bm{\theta}}}{\operatorname{argmin}}~{}\mathbb{E}_{(\bm{x},y)\sim\bm{S}}\left[\mathcal{L}(f(\bm{x};\bm{\theta}),y)+\frac{\alpha}{2}\|\bm{\theta}\|_{2}\right].
$$
...
*(a) Different pruning timing $t$: $\bm{m}_{{\color[rgb]{1,0,0}\definecolor[named]{pgfstrokecolor}{rgb}{1,0,0}t}}^{k}$*

![Refer to caption](../images/fig4b-x10.png)

*(b) Different pruning rate $k$: $\bm{m}_{t}^{{\color[rgb]{0,0,1}\definecolor[named]{pgfstrokecolor}{rgb}{0,0,1}k}}$*
...
1. Obtain lottery tickets after full generalization $\bm{m}_{t_{\text{gen}}}^{k}$.
2. Get weight $L_{p}$ norm ratio $r_{p}=\frac{\|\bm{\theta}_{0}\odot\bm{m}_{t_{\text{gen}}}^{k}\|_{p}}{\|\bm{\theta}_{0}\|_{p}}$
3. Create weights $\bm{\theta}_{0}\cdot r_{p}$ with the same $L_{p}$ norm as the grokked ticket.
...
| EP w/ WD | $\bm{0.82\pm 0.04}$ | $\bm{0.96\pm 0.01}$ | $0.99\pm 0.00$ | $1.00\pm 0.00$ |
...
: Training from $\bm{\theta}_{\text{mem}}$ using **Weight Decay** *with* weight update (same as base model).

: Training from $\bm{\theta}_{\text{mem}}$ using **Edge-Popup** *without* weight update.

: Training from $\bm{\theta}_{\text{mem}}$ using **Edge-Popup** and **Weight Decay** *with* weight update.
...
1. Randomly initialize a neural network $f(\bm{x};\bm{\theta}_{0})$.
2. Prune *p*% of the parameters in $\bm{\theta}_{0}$ according to the scores $S(\bm{\theta})$, creating a mask m.
3. Train the network from $\bm{\theta}_{0}\odot\bm{m}$.
...
$$
S(\bm{\theta})=\frac{\partial R}{\partial\bm{\theta}}\odot\bm{\theta}
$$

When the $R$ is the training loss $L$, the resulting synaptic saliency metric is equivalent to $|\frac{\partial L}{\partial\bm{\theta}}\odot\bm{\theta}|$ used in SNIP (Lee et al., 2019). $-(H\frac{\partial L}{\partial\bm{\theta}})\odot\bm{\theta}$ use in Grasp (Wang et al., 2020).Tanaka et al. (2020) proposed synflow algorithm $R_{SF}=1^{T}(\prod_{l=1}^{L}|\bm{\theta}^{|l|}|)1$. In section 4.2, all initial values were experimented with using the same weights and the same pruning rate.
...
$$
\mathrm{softmax}(\sigma((\bm{E}_{a}+\bm{E}_{b})W_{\text{in}})W_{\text{out}}W_{\text{unemb}}), \tag{4}
$$

where $\bm{E}_{a}$ and $\bm{E}_{b}$ are the input embeddings, $W_{\text{in}}$, $W_{\text{out}}$, and $W_{\text{unemb}}$ are the respective weight matrices, and $\sigma$ denotes the activation function. This architecture models the relationship between inputs in the Modular Addition task.
