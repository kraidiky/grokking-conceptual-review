...
**for** *$i=1$ **to** $N$* **do**
$\tau_{i},\Delta_{i}\leftarrow BO.\textsc{SuggestParameters}()$;
$\theta_{R}\leftarrow R(\theta,\tau_{i})$;
// Low-rank approximation
$\theta_{RQ}\leftarrow Q(\theta_{R},\Delta_{i})$;
// Quantization
**if** *$\left|\textsc{Loss}(\theta_{RQ})-\textsc{Loss}(\theta)\right|<\epsilon$* **then**
compressed_size $\leftarrow\textsc{compress}(\theta_{RQ})$;
**if** *compressed_size $<$ best_compressed_size* **then**
best_compressed_size $\leftarrow$ compressed_size;

end if
$BO.\textsc{UpdateModel}(\tau_{i},\Delta_{i},\text{compressed\_size})$;

**else**
$BO.\textsc{UpdateModel}(\tau_{i},\Delta_{i},\infty)$;
...
**if** *$\left|\textsc{Loss}(\text{quantized\_params})-\textsc{Loss}(\text{params})\right|<\epsilon$* **then**
