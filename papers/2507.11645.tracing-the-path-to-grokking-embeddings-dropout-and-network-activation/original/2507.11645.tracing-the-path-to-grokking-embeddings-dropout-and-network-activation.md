...
> `arxiv.org/html/2507.11645` отдаёт 404, ar5iv возвращает страницу аннотации
> (0 маркеров LaTeXML), а `arxiv.org/e-print/2507.11645` — сам PDF
...
> абзацы восстановлены по вордовскому отступу первой строки (x≈108 pt) и по
> буллетам SymbolMT. Метки страниц точны по построению: они не сопоставлены,
> а взяты из разбиения PDF; колонцифры (15 штук, Calibri 11 pt) не
> перенесены.
>
> Обе выключные формулы, (1) и (2), восстановлены вручную по отрисованным
> с. 3 и 5, а не по текстовому слою: у Cambria Math дроби, корни и пределы
> сумм рассыпаны по блокам с несовпадающими координатами. Строчная
> математика ($Y=(i+j)\%P$, $W{\sim}N(0,\sigma)$, обозначения векторов
> вложений) сверена с растром тех же строк. Иллюстрации — 21 панель
> рисунков 1–11 — извлечены встроенными файлами прямо из PDF
> (`extract_image`); панели (a)–(d) упорядочены по месту на странице.
> Подпись рисунка 3 в PDF слита с соседним абзацным блоком и отделена по
> смыслу; подписи рисунков 5–10 разорваны межстрочными блоками и склеены.
