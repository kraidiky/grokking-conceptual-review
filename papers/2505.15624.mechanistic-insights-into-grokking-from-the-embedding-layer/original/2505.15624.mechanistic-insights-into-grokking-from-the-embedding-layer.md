...
$$
\bm{h}_{1} =\sigma(\mathbf{W}_{1}x+\bm{b}_{1}),\quad\bm{h}_{2}=\mathbf{W}_{2}\bm{h}_{1}+\bm{b}_{2}, \\
\hat{\bm{y}} =\mathrm{Softmax}(\bm{h}_{2}). \tag{1}
$$

where $\bm{x}\in\mathbb{R}^{4}$ is the encoded input vector (with first and third entry $a$ and $b$, respectively), $\mathbf{W}_{1},\mathbf{W}_{2}$ are weight matrices, $\bm{b}_{1},\bm{b}_{2}$ are biases, $\sigma$ is the ReLU activation function, and $\hat{\bm{y}}$ represents the predicted output.
...
To overcome the challenges of non-linear separability, we introduced an embedding layer. Each token $x$ is mapped to a dense vector $\bm{e}_{x}$ through an embedding matrix $\mathbf{E}\in\mathbb{R}^{V\times d}$, where $d$ is the embedding dimension. Our input consists of 4 token embeddings of the form $\hat{\bm{e}}=[\bm{e}_{i},\bm{e}_{{}^{\prime}*^{\prime}},\bm{e}_{k},\bm{e}_{{}^{\prime}=^{\prime}}]^{\top}$, and the modified forward pass is:

$$
\bm{h}_{1} =\sigma(\mathbf{W}\hat{\bm{e}}+\bm{b}_{1}), \\
\bm{h}_{2} =\mathbf{W}_{2}\bm{h}_{1}+\bm{b}_{2},\quad\hat{\bm{y}}=\mathrm{Softmax}(\bm{h}_{2}), \tag{2}
$$
...
Let the loss function of the model be $\mathcal{L}(\theta,\mathbf{E})$, where $\theta$ is model parameters other than embedding weights. Let $\bm{e}_{i,t}$ denote the embedding vector for token $i$ at step $t$. Under stochastic gradient descent (SGD) with weight decay $\lambda$, the embedding update rule is:

$$
\bm{e}_{i,t+1}-\bm{e}_{i,t}=-\eta\lambda\bm{e}_{i,t}-\eta\nabla_{\bm{e}_{i,t}}\mathcal{L}, \tag{3}
$$
...
$$
\mathbb{E}[\bm{e}_{i,t+1}-\bm{e}_{i,t}]=-\eta\lambda\bm{e}_{i,t}-\eta p_{i}\nabla_{\bm{e}_{i,t}}\mathcal{L}. \tag{4}
$$
...
$$
\mathcal{L}(\theta_{t+1}, \{\bm{e}_{i,t+1}\})\leq\mathcal{L}(\theta_{t},\{\bm{e}_{i,t}\})+\langle\nabla\mathcal{L},\Delta\rangle+\frac{\beta}{2}\|\Delta\|^{2}.
$$

where $\Delta=(\theta_{t+1}-\theta_{t},\bm{e}_{i,t+1}-\bm{e}_{i,t})$.

###### p6-4

Denote $\mathcal{L}_{t}:=\mathcal{L}(\theta_{t},\{\bm{e}_{i,t}\})$ then taking expectations over randomness of batch sampling leads to the following expected update:

$$
\mathbb{E} [\mathcal{L}_{t+1}-\mathcal{L}_{t}]\leq\nabla_{\theta_{t}}\mathcal{L}^{T}(\theta_{t+1}+\theta_{t}) \\
-\sum_{i=1}^{V}\nabla_{\bm{e}_{i,t}}\mathcal{L}^{T}\mathbb{E}(\bm{e}_{i,t+1}-\bm{e}_{i,t})+\frac{\beta}{2}\|\Delta\|^{2}, \tag{5}
$$

###### p6-5

Substituting the embedding update based on equation 4 into the smoothness inequality, $\displaystyle\mathbb{E}$ $\displaystyle[\mathcal{L}_{t+1}-\mathcal{L}_{t}]\leq\nabla_{\theta_{t}}\mathcal{L}^{T}(\theta_{t+1}-\theta_{t})$ $\displaystyle-\eta\sum_{i=1}^{V}\left(p_{i}\|\nabla_{\bm{e}_{i,t}}\mathcal{L}\|^{2}+\lambda\bm{e}_{i,t}^{T}\nabla_{\bm{e}_{i,t}}\mathcal{L}\right)+\frac{\beta}{2}\|\Delta\|^{2},$ (6)
...
Stabilization (or convergence) occurs when the embedding $\mathbf{e}_{i}$ reaches a steady state where the updates become negligibly small, i.e., when the change in the embedding $\|\bm{e}_{i,t+1}-\bm{e}_{i,t}\|$ is approximately zero. This condition implies that, $(\eta\lambda)\bm{e}_{i,t}\approx\eta p_{i}\nabla_{\mathbf{e}_{i}}\mathcal{L}.$ from equation 4.
...
$$
\mathbf{H}\mathbf{v}_{k}=\nabla_{\bm{\theta}}\left(\nabla_{\bm{\theta}}\mathcal{L}\cdot\mathbf{v}_{k}\right),
$$

 where $\bm{\theta}=[\mathbf{E},\mathbf{W}]$.
