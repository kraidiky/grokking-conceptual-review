...
To train models on modular arithmetic tasks, we construct input-label pairs by one-hot encoding the input and label integers. Specifically, for every pair $a,b\in\mathbb{Z}_{p}$, we write the input as $\bm{e}_{a}\oplus\bm{e}_{b}\in\mathbb{R}^{2p}$ and the output as $\bm{e}_{f^{*}(a,b)}\in\mathbb{R}^{p}$, where $\bm{e}_{i}\in\mathbb{R}^{p}$ is the $i$-th standard basis vector in $p$ dimensions and $\oplus$ denotes concatenation. For example, for addition modulo $p=3$, the equality $1+2=0\,\mathrm{mod}\,3$ is encoded as an input/label pair of vectors in $\mathbb{R}^{6}$ and $\mathbb{R}^{3}$, respectively:
...
for $j\in[p]$. We write the $\ell$-fold composition of this map $\sigma^{\ell}(u)\in\mathbb{R}^{p}$ with entries $[\sigma^{\ell}(u)]_{j}=u_{j-\ell\,\mathrm{mod}\,p}$. A circulant matrix $C\in\mathbb{R}^{p\times p}$ is determined by a vector $\bm{c}=[c_{0},\ldots,c_{p-1}]\in\mathbb{R}^{p}$, and has form:

$$
C=\begin{pmatrix}\rotatebox[origin]{90.0}{$|$}&\bm{c}&\rotatebox[origin]{90.0}{$|$}\\
\rotatebox[origin]{90.0}{$|$}&\sigma(\bm{c})&\rotatebox[origin]{90.0}{$|$}\\
\vdots&\vdots&\vdots\\
\rotatebox[origin]{90.0}{$|$}&\sigma^{p-1}(\bm{c})&\rotatebox[origin]{90.0}{$|$}\end{pmatrix}~.
$$

Namely, circulant matrices are those in which each row is shifted one element to the right of its preceding row. Hankel matrices are also determined by a vector $\bm{c}$, but the rows are $\bm{c},\sigma^{-1}(\bm{c}),\ldots,\sigma^{-(p-1)}(\bm{c})$. While circulant matrices have constant diagonals, Hankel matrices have constant anti-diagonals. To ease terminology, we will use the word circulant to refer to Hankel or circulant matrices as previously defined.
...
*where $A,C\in\mathbb{R}^{p\times p}$ and $C$ is an asymmetric circulant matrix (i.e. $C$ is not a degenerate circulant such as $C=I$ or $C=\bm{1}\bm{1}^{\top}$). Furthermore, we note that $A$ is of the form $c_{1}I+c_{2}\bm{1}\bm{1}^{\top}$ for constants $c_{1},c_{2}$.*
...
$$
\mathrm{Var}(\bm{v})=\sum_{j=0}^{p-1}(v_{j}-\mathbb{E}\bm{v})^{2}
$$

be the variance of a vector $\bm{v}$. If $A[j]$ denotes the $j$-th column of $A$, we define circulant deviation $\mathcal{D}$ as
...
3. Return $\sqrt{p}~\cdot~\langle\widehat{x}_{[1]}\odot\widehat{x}_{[2]},F\bm{e}_{\ell}\rangle_{\mathbb{C}}$ where $\bm{e}_{\ell}$ denotes $\ell$-th standard basis vector and $\langle\cdot,\cdot\rangle_{\mathbb{C}}$ denotes the complex inner product (see Eq. (1)).

This algorithmic process can be written concisely in the following equation:

$$
y_{\mathrm{add}}(x;\ell)=\sqrt{p}\cdot\left\langle Fx_{[1]}\odot Fx_{[2]},F\bm{e}_{\ell}\right\rangle_{\mathbb{C}}~. \tag{11}
$$

Note that for $x=\bm{e}_{a}\oplus\bm{e}_{b}$, the second step of the FMA reduces to

$$
F\bm{e}_{a}\odot F\bm{e}_{b}=\frac{1}{\sqrt{p}}F\bm{e}_{(a+b)\,\mathrm{mod}\,p}~. \tag{12}
$$

Using the fact that $F$ is a unitary matrix, the output of the FMA is given by

$$
\sqrt{p}\cdot\left\langle\frac{1}{\sqrt{p}}F\bm{e}_{(a+b)\,\mathrm{mod}\,p},F\bm{e}_{\ell}\right\rangle_{\mathbb{C}}=\bm{e}_{(a+b)\,\mathrm{mod}\,p}^{\top}F^{\top}\bar{F}\bm{e}_{\ell}=\bm{e}_{(a+b)\,\mathrm{mod}\,p}^{\top}\bm{e}_{\ell}=\mathbbm{1}_{\{(a+b)\,\mathrm{mod}\,p=\ell\}}~. \tag{13}
$$

Thus, the output of the FMA is a vector $\bm{e}_{(a+b)\,\mathrm{mod}\,p}$, which is equivalent to modular addition. We provide an example of this algorithm for $p=3$ in Appendix F.
...
$$
y_{\mathrm{sub}}(x;\ell)=\sqrt{p}\cdot\left\langle Fx_{[1]}\odot F\bm{e}_{p-\ell-1},Fx_{[2]}\right\rangle_{\mathbb{C}}~. \tag{14}
$$
...
We now provide an example of the FMA for $p=3$. Let $x=\bm{e}_{1}\oplus\bm{e}_{2}$. In this case, we expect the FMA to output the vector $\bm{e}_{0}$ since $(1+2)\,\mathrm{mod}\,3=0$. Following the first step of the FMA, we compute

$$
\widehat{x}_{[1]}=F\bm{e}_{1}=\frac{1}{\sqrt{3}}[1,\omega,\omega^{2}]^{\top}~~;~~\widehat{x}_{[2]}=F\bm{e}_{2}=\frac{1}{\sqrt{3}}[1,\omega^{2},\omega^{4}]^{\top}~, \tag{16}
$$

which are the first and second columns of $F$, respectively. Then their element-wise product is given by

$$
F\bm{e}_{1}\odot F\bm{e}_{2}=\frac{1}{3}[1,\omega^{3},\omega^{6}]^{\top}=\frac{1}{3}[1,1,1]^{\top}=\frac{1}{\sqrt{3}}F\bm{e}_{0}~, \tag{17}
$$

which is $\frac{1}{\sqrt{3}}$ times the first column of the DFT matrix. Finally, we compute the outputs $\sqrt{3}\left\langle\frac{1}{\sqrt{3}}F\bm{e}_{0},F\bm{e}_{\ell}\right\rangle_{\mathbb{C}}$ for each $\ell\in\{0,1,2\}$. As $F$ is unitary, $y_{\mathrm{add}}(\bm{e}_{1}\oplus\bm{e}_{2};\ell)=\mathbbm{1}_{\left\{1+2=\ell\,\mathrm{mod}\,3\right\}}$, so that coordinate $0$ of the output will have value $1$, and all other coordinates have value $0$.
...
$$
f_{\ell}(x)=\sum_{a,b=0}^{p-1}\alpha^{(\ell)}_{a,b}k\left(x,\bm{e}_{a}\oplus\bm{e}_{b};M_{\ell}\right)~,
$$

where we have re-written the index into kernel coefficients for class $\ell$, $\alpha^{(\ell)}\in\mathbb{R}^{p\times p}$, so that the coefficients are multi-indexed by the first and second digit. Specifically, now $\alpha^{(\ell)}_{a,b}$ is the kernel coefficient corresponding to the representer $k(\cdot,x)$ for input point $x=\bm{e}_{a}\oplus\bm{e}_{b}$. Recall we use a quadratic kernel, $k(x,z;M_{\ell})=(x^{\top}M_{\ell}z)^{2}$. In this case, the kernel predictor simplifies to,

$$
f_{\ell}(x)=\sum_{a,b=0}^{p-1}\alpha^{(\ell)}_{a,b}\left(x_{[1]}^{\top}C^{\ell}\bm{e}_{b}+\bm{e}_{a}^{\top}C^{\ell}x_{[2]}\right)^{2}~.
$$
...
For $x=\bm{e}_{a^{\prime}}\oplus\bm{e}_{b^{\prime}}$, i.e. $x$ in the discrete dataset, we have,
...
=\bm{e}_{b^{\prime}-\ell}^{\top}\alpha^{(\ell)}\bm{1}+\bm{1}^{\top}\alpha^{(\ell)}\bm{e}_{a^{\prime}+\ell}+2\bm{e}_{b^{\prime}-\ell}^{\top}\alpha^{(\ell)}\bm{e}_{a^{\prime}+\ell} \\
=\bm{e}_{b^{\prime}}^{\top}C^{-\ell}\alpha^{(\ell)}\bm{1}+\bm{1}^{\top}\alpha^{(\ell)}C^{-\ell}\bm{e}_{a^{\prime}}+2\bm{e}_{b^{\prime}}^{\top}C^{-\ell}\alpha^{(\ell)}C^{-\ell}\bm{e}_{a^{\prime}} \\
=\bm{e}_{b^{\prime}}^{\top}\left(C^{-\ell}\alpha\bm{1}\bm{1}^{\top}+\bm{1}\bm{1}^{\top}\alpha C^{-\ell}+2C^{-\ell}\alpha C^{-\ell}\right)\bm{e}_{a^{\prime}}~,
$$

###### p23-3

where $\delta_{(u,v)}=\mathbbm{1}_{\left\{u=v\right\}}$. Let $f_{\ell}(X)\in\mathbb{R}^{p\times p}$ be the matrix of function values of $f_{\ell}$, where $[f_{\ell}(X)]_{a,b}=f_{\ell}(\bm{e}_{a}\oplus\bm{e}_{b})$, and, therefore, $f_{\ell}(\bm{e}_{a}\oplus\bm{e}_{b})=\bm{e}_{a}^{\top}f_{\ell}(X)\bm{e}_{b}$. Then, to solve for $\alpha^{(\ell)}$, we need to solve the system of equations for $\alpha$,

$$
f_{\ell}(X)=\left(C^{-\ell}\alpha\bm{1}\bm{1}^{\top}+\bm{1}\bm{1}^{\top}\alpha C^{-\ell}+2C^{-\ell}\alpha C^{-\ell}\right)^{\top}=C^{-\ell} \\
\iff C^{-\ell}\alpha\bm{1}\bm{1}^{\top}+\bm{1}\bm{1}^{\top}\alpha C^{-\ell}+2C^{-\ell}\alpha C^{-\ell}=C^{\ell}
$$

Note, by left-multiplying both sides by $C^{-\ell}$, we see this equation holds iff,

$$
C^{-2\ell}\alpha\bm{1}\bm{1}^{\top}+\bm{1}\bm{1}^{\top}\alpha C^{-\ell}+2C^{-2\ell}\alpha C^{-\ell}=I~.
$$

Note the solution is unique as the kernel matrix is full rank. We posit the solution $\alpha$ such that $C^{-2\ell}\alpha C^{-\ell}=\frac{1}{2}I+\lambda\bm{1}\bm{1}^{\top}$, which is $\alpha=\frac{1}{2}C^{3\ell}+\lambda\bm{1}\bm{1}^{\top}$. Then, solving for $\lambda$, we require,

$$
\bm{1}\bm{1}^{\top}+2p\lambda\bm{1}\bm{1}^{\top}+2\lambda\bm{1}\bm{1}^{\top}=0~,
$$

which implies $\lambda=-\frac{2}{2p+2}$. Substituting this value of $\lambda$ and simplifying, we see finally that $f_{\ell}(x)=x_{[1]}^{\top}C^{-\ell}x_{[2]}$. Therefore, using that circulant matrices are diagonalized by $C=\sqrt{p}FD\bar{F}^{\top}$ (Lemma G.2) and $\bar{F}^{\top}F=I$, where $D=\textbf{diag}\left(F\bm{e}_{1}\right)$, we derive,

$$
f_{\ell}(x) =\sqrt{p}\cdot x_{[1]}^{\top}FD^{-\ell}\bar{F}^{\top}x_{[2]} \\
=\sqrt{p}\cdot x_{[1]}^{\top}F\textbf{diag}\left(F\bm{e}_{p-\ell-1}\right)\bar{F}^{\top}x_{[2]} \\
=\sqrt{p}\cdot\left\langle Fx_{[1]}\odot F\bm{e}_{p-\ell-1},Fx_{[2]}\right\rangle_{\mathbb{C}}
$$
...
We define the “encoding” map $\Phi:\mathbb{R}^{p}\to\mathbb{C}$ as follows. For a vector $\bm{a}=[a_{0},\ldots,a_{p-1}]$,

$$
\Phi(\bm{a})=\sum_{k=0}^{p-1}a_{k}\exp\left(\frac{k2\pi i}{p}\right)~.
$$

Notice that $\Phi$ is a linear map such that $\Phi(\bm{e}_{k})=\exp\left(\frac{k2\pi i}{p}\right)$. Notice also that $\Phi$ is partially invertible with the “decoding” map $\Psi:\mathbb{C}\to\mathbb{R}^{p}$.
...
$$
\Psi\left(\exp\left(\frac{k\cdot 2\pi i}{p}\right)\right)=\bm{e}_{k}~. \tag{19}
$$
...
$$
\Phi(\bm{e}_{j},\bm{e}_{k})=(\Phi(\bm{e}_{j}),\Phi(\bm{e}_{k}))~.
$$

This is still a linear map but now to $\mathbb{C}^{2}$.

Consider now a quadratic map $\bm{M}$ on $\mathbb{C}^{2}\to\mathbb{C}$ given by complex multiplication:

$$
\bm{M}(z_{1},z_{2})=z_{1}z_{2}~.
$$

It is clear that the composition $\Psi\bm{M}\Phi$ implements modular addition

$$
\Psi\bm{M}\Phi(\bm{e}_{j},\bm{e}_{k})=\bm{e}_{(j+k)\,\mathrm{mod}\,p}
$$

Furthermore, since $\Phi$ is a liner map to a four-dimensional space, the AGOP of the composition $\Psi\bm{M}\Phi$ is of rank $4$.

#### Multiplication

###### p24-8

The construction is for multiplication is very similar with modifications which we sketch below. We first re-order the non-zero coordinates by the discrete logarithm with base equal to a generator of the multiplicative group $\bm{e}_{g}$ (see Appendix C), while keeping the order of index $0$. Then, we modify $\Phi$ to remove index $a_{0}$ from the sum for inputs $\bm{a}$. Thus for multiplication,

$$
\Phi(\bm{a})=\sum_{k=1}^{p-1}a_{k}\exp\left(\frac{k\cdot 2\pi i}{p-1}\right)~,
$$

###### p24-9

Hence that $\Phi(\bm{e}_{0})=0$, $\Phi(\bm{e}_{g})=\exp\left(\frac{2\pi i}{p-1}\right)$ and $\Phi(\bm{e}_{g^{k}})=\exp\left(\frac{k\cdot 2\pi i}{p-1}\right)$. We extend $\Phi$ to $\mathbb{R}^{p}\times\mathbb{R}^{p}$ as in Eq. 19 above. Note that $\Phi$ and the re-ordering together are still a linear map of rank $4$.
...
$\bm{M}$ is still defined as above. It is easy to check that the composition of $\Psi\bm{M}\Phi$ with reordering implements modular multiplication modulo $p$ and furthermore, the AGOP will also be of rank $4$.
