...
2. We prove that networks with appropriate regularization can generalize with many fewer samples: $\bm{\operatorname{\mathcal{O}}(1/p)}$ portion of all possible data points for square loss generalization on the regression task (Section 3.2), and $\bm{\tilde{\operatorname{\mathcal{O}}}(1/p^{1/3})}$ for zero-one loss generalization on classification (Section 4.2).
...
In the regression setting (Section 3), we instead aim to learn for each tuple $(a,b,c)$, if $a+b\equiv c\pmod{p}$, that is, map ${\bm{x}}=(e_{a},e_{b},e_{c})$ to the scalar $y=p\,\mathbf{1}(a+b\equiv c\pmod{p})$ using $g(\theta;(e_{a},e_{b},e_{c})))\triangleq e_{c}^{\top}f(\theta;(e_{a},e_{b}))$.222This scaling implies that the predictor $\Psi_{\bm{0}}(\cdot)=0$ has population square loss $p$; this scaling allows bounded $\left\lVert\theta\right\rVert_{\infty}$. Here $\mathcal{X}=\mathcal{Z}^{3}$, $\mathcal{Y}=\{0,p\}$, and $N=p^{3}$; we train the model $g(\theta;{\bm{x}})=\langle e_{c},f(\theta;(e_{a},e_{b}))\rangle$ to minimize the square loss.

In either setting, we use $\mathcal{D}$ to denote the distribution over $\mathcal{X}\times\mathcal{Y}$ which is uniform over all $N$ possible input-output pairs, while $\mathcal{D}_{\mathrm{train}}=\left\{({\bm{x}}_{i},y_{i})\right\}_{i=1}^{n}$ is the training dataset of size $n$. Given parameters $\theta$, we use $\Psi(\theta;\cdot):\mathcal{X}\to\hat{\mathcal{Y}}$ to denote the predictor ($g(\theta;\cdot)$ for regression, $f(\theta;\cdot)$ for classification), we train the model with gradient descent with tiny $\ell_{\infty}$ regularization,333More details about the regularization strength can be found in Appendix A. which is intended to emulate the implicit bias of Adam (AdamW) which might lead to grokking. In general, we define loss as
...
For any $\mathcal{D}_{\mathrm{train}}=\{({\bm{x}}_{i},y_{i})\}_{i=1}^{n}$ and transformation $T\in{\mathcal{G}}_{\mathcal{X}}$, let $\mathcal{D}_{\mathrm{train}}^{T}$ be the transformed dataset $\{(T({\bm{x}}_{i}),y_{i})\}_{i=1}^{n}$, and ${\mathcal{A}}_{K}$ be the kernel regression algorithm w.r.t. $K$. For any ${\bm{x}}$, we have that
...
*Initialize the network of Section 2 with any values for $V$, and entries of $W$ all independently $\operatorname{Uniform}([-s_{W},s_{W}])$ for some $s_{W}>0$. Let the most frequent value of $c\in[p]$ appear $\rho_{c}$ times. Then, if $h>36\rho_{c}\log(n/\delta)$, it holds with probability at least $1-\delta$ over the random initialization of $W$ that kernel regression using the network’s empirical neural tangent kernel can achieve zero training loss for *any* target labels such that if ${\bm{x}}_{i}={\bm{x}}_{j}$ then $y_{i}=y_{j}$. Conversely, if $h<n/(3p)$, there exist target labels (with $y_{i}=y_{j}$ when ${\bm{x}}_{i}={\bm{x}}_{j}$) for which this method cannot achieve zero training loss.*
...
More formally, we can define the following correspondence: given one data point in classification $({\bm{x}},y)\in\mathbb{R}^{2p}\times\mathbb{R}^{p}$, we can view it as $p$ data points in regression, $\{(({\bm{x}},e_{i}),y_{i})\}_{i=1}^{p}$, denoted by $F({\bm{x}},y)$. Similarly, any function $\Psi$ mapping from ${\bm{x}}=\{(e_{i},e_{j})\}$ to $\mathbb{R}^{p}$ can be viewed as a function mapping $\{(e_{i},e_{j},e_{k})\}$ to $\mathbb{R}$ by defining $\Psi^{\prime}({\bm{x}},e_{k})=[\Psi({\bm{x}})]_{k}$. Moreover, these two functions $\Psi$ and $\Psi^{\prime}$ share the same population $\ell_{2}$ loss. Under this view, matrix-valued kernel learning with $n$ classification data points is exactly the same as scalar-valued kernel learning with $np$ regression data points.
...
To analyze the behaviour of the network in the rich regime, we first introduce the notion of margin and elaborate on the margin-maximization implicit bias of gradient descent. For a multi-class classification problem with $p$ classes and a fixed network $f$, the margin of a data point $({\bm{x}},y)$ is
...
If the training set contains duplicate ${\bm{x}}$, say ${\bm{x}}_{i}={\bm{x}}_{j}$ for $i\neq j$, then $\mathbf{K}$ cannot be full rank – the $i$th and $j$th rows are necessarily identical. Of course, there also exist unattainable targets; just set $y_{i}\neq y_{j}$. Call a labeling *consistent* if for all $i$ and $j$ such that ${\bm{x}}_{i}={\bm{x}}_{j}$, $y_{i}=y_{j}$. A learning algorithm can achieve any consistent labeling if and only if that algorithm applied to the largest distinct subset (e.g. removing $j$ but keeping $i$) can achieve any labeling. Thus, we assume without loss of generality in the remainder of this section that the training set contains no duplicates.
...
where $\mathbf{K}^{\dagger}$ is the Moore-Penrose pseudoinverse of the $n\times n$ kernel matrix $[K({\bm{x}}_{i},{\bm{x}}_{j})]_{ij}$, and $\mathbf{y^{\prime}}$ has $i$th entry $y_{i}-f_{0}({\bm{x}}_{i})$. By changing variables between $\mathbf{y^{\prime}}$ and $\mathbf{y}$, we need not explicitly consider $f_{0}$ in the following two results.
...
This well-known result follows from the fact that $[\hat{f}({\bm{x}}_{i})]_{i}=\mathbf{K}\mathbf{K}^{\dagger}\mathbf{y}$. If $\mathbf{K}$ is strictly positive definite, $\mathbf{K}^{\dagger}=\mathbf{K}^{-1}$, and so $[\hat{f}(x_{i})]_{i}=\mathbf{y}$. ∎
...
for arbitrary nonrandom ${\bm{x}}=(e_{a},e_{b},e_{c})$ and ${\bm{x}}^{\prime}=(e_{a^{\prime}},e_{b^{\prime}},e_{c^{\prime}})$, where all expectations will be over the relevant parameters $\{Q_{ka}:a\in[p]\}\cup\{R_{kb}:b\in[p]\}$.
...
It remains to show that the final component of the kernel is strictly positive definite. Noticing that $\mathbf{1}(a=a^{\prime},b=b^{\prime},c=c^{\prime})=\mathbf{1}(x=x^{\prime})$, if the $\{{\bm{x}}_{i}:i\in[n]\}$ are distinct, the expected kernel matrix is $\operatorname*{\mathbb{E}}\mathbf{L}_{\theta_{k}}=\mathbf{M}+4\sigma_{W}^{4}\mathbf{I}$. Since $\mathbf{M}$ is positive semi-definite, this has minimum eigenvalue at least $4\sigma_{W}^{4}>0$, as desired. ∎
...
Recall the decomposition of $K_{\theta}({\bm{x}},{\bm{x}}^{\prime})=J_{\theta}({\bm{x}},{\bm{x}}^{\prime})+\sum_{k=1}^{h}L_{\theta_{k}}({\bm{x}},{\bm{x}}^{\prime})$ from (B.2), and the corresponding $n\times n$ matrices $\mathbf{K}_{\theta}=[K_{\theta}({\bm{x}}_{i},{\bm{x}}_{j})]_{ij}$, $\mathbf{J}_{\theta}=[J_{\theta}({\bm{x}}_{i},{\bm{x}}_{j})]_{ij}$, and $\mathbf{L}_{\theta_{k}}=[L_{\theta_{k}}({\bm{x}}_{i},{\bm{x}}_{j})]_{ij}$, so that $\mathbf{K}_{\theta}=\mathbf{J}_{\theta}+\sum_{k=1}^{h}\mathbf{L}_{\theta_{k}}$. As shown in the proof of Proposition B.3, $\mathbf{J}_{\theta}$ is positive semi-definite; we now wish to show that the sum of $h$ iid matrices $\mathbf{L}_{\theta_{k}}$, whose mean $h\operatorname*{\mathbb{E}}\mathbf{L}_{\theta_{k}}$ has minimum eigenvalue $\mu_{\min}:=4h\sigma_{W}^{4}$, is likely to be full-rank. We can do so by applying Lemma B.5, a direct corollary of matrix Chernoff bounds, if we additionally have an almost sure upper bound on the operator norm $\lVert\mathbf{L}_{\theta_{k}}\rVert$.

Notate the input ${\bm{x}}_{i}$ as $(e_{a_{i}},e_{b_{i}},e_{c_{i}})$ for $a_{i},b_{i},c_{i}\in[p]$. Using (B.3), we can write
...
In this section we present the formal version of Theorem 3.4 alongside a proof of it. Note that a kernel-based predictor $h$ on a training data $\left\{({\bm{x}}_{i},y_{i})\right\}_{i=1}^{n}$ can be expressed as $h(x)=\sum_{i=1}^{n}\lambda_{i}K({\bm{x}}_{i},x)$ where $\lambda_{i};i\!\in\![n]$ are constants. Assuming that the kernel’s feature maps are of dimension $d$, the predictions are linear combinations of $d$-dimensional feature maps. We first present a general proof that every permutation invariant kernel requires $\Omega(p^{2})$ training points to outperform the null predictor in terms of $\ell_{2}$ loss, and then show that this theorem applies to the distribution of empirical NTKs at initialization.

### D.1 Notation

We use $[p]$ to denote the set $\{1,\ldots,p\}$. We use $\mathbb{S}_{p}$ to denote the permutation groups over $p$ elements, ${\mathbb{S}}_{m}$ as permutation group over $m$ elements and $\mathsf{id}$ is the identity mapping. For any nonempty set $\mathcal{X}$, a symmetric function $K:\mathcal{X}\times\mathcal{X}\to\mathbb{R}$ is called a positive semi-definite kernel (p.s.d) kernel on $\mathcal{X}$ if for any $n\in\mathbb{N}$, any ${\bm{x}}_{1},\ldots,{\bm{x}}_{n}$ and $\lambda_{1},\ldots,\lambda_{n}\in\mathbb{R}$, it holds that $\sum_{i=1}^{n}\sum_{j=1}^{n}\lambda_{i}\lambda_{j}K({\bm{x}}_{i},{\bm{x}}_{j})\geq 0$. For a subspace $V$ of $\mathbb{R}^{n}$ and vector $x\in\mathbb{R}^{n}$, we define $\mathsf{dist}(x,V)\triangleq\min_{v\in V}\left\lVert x-v\right\rVert_{2}$.
...
Because $\mathcal{A}$ is a kernel method, we have for any $({\bm{x}}_{i},y_{i})_{i=1}^{n}$ and $\sigma_{1},\sigma_{2},\sigma_{3}\in{\mathbb{S}}_{p}$
